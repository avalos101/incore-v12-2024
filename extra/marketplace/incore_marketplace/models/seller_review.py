...
class SellerReview(models.Model):
    ...
    def action_review_helfull_fun(self):
        self.ensure_one()
        action = self.env.ref('incore_marketplace.action_reviews_helpful_list')
        list_view_id = self.env['ir.model.data'].xmlid_to_res_id(
            'incore_marketplace.seller_review_help_tree_view')
        form_view_id = self.env['ir.model.data'].xmlid_to_res_id(
            'incore_marketplace.wk_seller_review_help_Form_view')
        return {
            'name': action.name,
            'help': action.help,
            'type': action.type,
    ...
    def action_review_not_helpful_fun(self):
        self.ensure_one()
        action = self.env.ref(
            'incore_marketplace.action_reviews_not_helpful_list')
        list_view_id = self.env['ir.model.data'].xmlid_to_res_id(
            'incore_marketplace.seller_review_help_tree_view')
        form_view_id = self.env['ir.model.data'].xmlid_to_res_id(
            'incore_marketplace.wk_seller_review_help_Form_view')
