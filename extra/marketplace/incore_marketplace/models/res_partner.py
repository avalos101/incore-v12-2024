...
from incore.osv.orm import setup_modifiers
import decimal,re
from incore.exceptions import except_orm, Warning, RedirectWarning, UserError
import logging
...
class ResPartner(models.Model):
    ...
    def _set_payment_method(self):
        ...
        try:
            payment_method_cheque_id = self.env['ir.model.data'].get_object_reference(
                'incore_marketplace', 'marketplace_seller_payment_method_data2')
        ...
        try:
            payment_method_bank_transfer_id = self.env['ir.model.data'].get_object_reference(
                'incore_marketplace', 'marketplace_seller_payment_method_data5')
            if payment_method_cheque_id:
    ...
    commission = fields.Float(string="Default Sale Commission", default=lambda self: self.env['ir.default'].get(
        'res.config.settings', 'global_commission'), read=['incore_marketplace.marketplace_draft_seller_group'], write=['incore_marketplace.marketplace_manager_group'], copy=False, track_visibility='onchange')
    ...
    seller_payment_limit = fields.Integer(string="Seller Payment Limit", default=lambda self: self.env['ir.default'].get(
        'res.config.settings', 'seller_payment_limit'), read=['incore_marketplace.marketplace_draft_seller_group'], write=['incore_marketplace.marketplace_manager_group'], copy=False, track_visibility='onchange')
    next_payment_request = fields.Integer(string="Next Payment Request", default=lambda self: self.env['ir.default'].get(
        'res.config.settings', 'next_payment_requset'), read=['incore_marketplace.marketplace_draft_seller_group'], write=['incore_marketplace.marketplace_manager_group'], copy=False, track_visibility='onchange')
    ...
    def write(self, vals):
        ...
        new_seller = False
        if not self.env.user.has_group('incore_marketplace.marketplace_manager_group'):
    ...
    def read(self, fields=None, load='_classic_read'):
        ...
        module_installed = irmodule_obj.sudo().search([('name', 'in', ['incore_marketplace']), ('state', 'in', ['installed'])])
        if module_installed:
    ...
    def action_seller_sol(self):
        self.ensure_one()
        action = self.env.ref('incore_marketplace.wk_seller_slae_order_line_action')
        list_view_id = self.env['ir.model.data'].xmlid_to_res_id(
            'incore_marketplace.wk_seller_product_order_line_tree_view')
        form_view_id = self.env['ir.model.data'].xmlid_to_res_id(
            'incore_marketplace.wk_seller_product_order_line_form_view')
        return {
            'name': action.name,
            'help': action.help,
            'type': action.type,
    ...
    def change_seller_group(self, set_to_group=None):
        ...
        if not login_user_obj.has_group('incore_marketplace.marketplace_officer_group'):
        ...
        for seller in self.filtered(lambda o: o.seller == True):
            ...
            pending_seller_group_obj = self.env.ref('incore_marketplace.marketplace_draft_seller_group')
            seller_group_obj = self.env.ref('incore_marketplace.marketplace_seller_group')
            if set_to_group == "seller":
                #First check seller user realy belongs to draft seller group(marketplace_draft_seller_group) or not
                if seller_user.has_group("incore_marketplace.marketplace_draft_seller_group"):
                ...
                if seller_user.has_group("incore_marketplace.marketplace_seller_group"):
    ...
    def action_avg_seller_review_fun(self):
        self.ensure_one()
        action = self.env.ref('incore_marketplace.action_seller_review')
        list_view_id = self.env['ir.model.data'].xmlid_to_res_id(
            'incore_marketplace.mp_seller_review_tree_view_webkul2')
        form_view_id = self.env['ir.model.data'].xmlid_to_res_id(
            'incore_marketplace.mp_seller_review_form_view_webkul2')
        return {
            'name': action.name,
            'help': action.help,
            'type': action.type,
    ...
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        ...
        manager_group = self.env['ir.model.data'].get_object_reference(
            'incore_marketplace', 'marketplace_officer_group')[1]
    ...
    def register_partner_as_a_seller(self):
        for rec in self:
            ...
            return {
                ...
                'view_mode':'form',
                'view_type':'form',
                'view_id':rec.env.ref('incore_marketplace.seller_registration_wizard_form').id,
                'context' : context,
                'target':'new',
            }
