...
class IrUiMenu(models.Model):
    ...
    def hide_mp_menus_to_user(self, menu_data):
        ...
        if core_website_menu_id in menu_data:
            menu_ids.extend((
                self.env.ref('incore_marketplace.wk_seller_website_menu').id,
            ))
        else:
            menu_ids = []
        ...
        if officer_group in groups_ids:
            try:
                menu_ids.extend((self.env.ref('incore_marketplace.wk_seller_payment_request').id,))
    ...
    def update_mp_menus(self, res):
        if not res:
            return res
        if self.env.user.has_group('incore_marketplace.marketplace_draft_seller_group') and not self.env.user.has_group('incore_marketplace.marketplace_officer_group'):
            seller_menu_id = self.env['ir.model.data'].get_object_reference(
                'incore_marketplace', 'wk_seller_dashboard_menu1_sub_menu1')[1]
            seller_shops_menu_id = self.env['ir.model.data'].get_object_reference(
                'incore_marketplace', 'wk_seller_dashboard_menu1_sub_menu2')[1]
