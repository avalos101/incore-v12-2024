...
class marketplace_dashboard(models.Model):
    ...
    def is_user_seller(self):
        ...
        seller_group = self.env['ir.model.data'].get_object_reference(
            'incore_marketplace', 'marketplace_draft_seller_group')[1]
    ...
    def _is_seller_or_manager(self):
        lst = []
        if self._uid:
            seller_groups = self.env['ir.model.data'].sudo().xmlid_to_object('incore_marketplace.marketplace_seller_group')
            manager_group = self.env['ir.model.data'].sudo().xmlid_to_object('incore_marketplace.marketplace_officer_group')
    ...
    def _get_pending_count(self):
        ...
        if self.state == 'order':
            ...
            if self.is_seller:
                obj = self.env['sale.order.line'].search(
                    [('marketplace_seller_id', '=',user_obj.partner_id.id), ('marketplace_state', '=', 'new')])
            else:
                obj = self.env['sale.order.line'].search(
                    [('marketplace_seller_id', '!=', False), ('marketplace_state', '=', 'new')])
            self.count_product_pending = len(obj)
        if self.state == 'payment':
            obj = self.env['seller.payment'].search(
