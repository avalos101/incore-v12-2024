...
# Please go through this link for complete license : https://store.incore.co/license.html
#
# DISCLAIMER
#
# Do not edit or add to this file if you wish to upgrade this module to newer
# versions in the future. If you wish to customize this module for your
# needs please refer to https://store.incore.co/customisation-guidelines/ for more information.
#
# @Author        : inCore.  (<support@incore.co>)
# @Copyright (c) : 2010-2017 Present inCore.  (<https://incore.co/>)
# @License       : https://store.incore.co/license.html
#
##########################################################################


from incore import models,fields,api,http
...
class SellerConfirmation(models.TransientModel):
	...
	def get_seller_profile(self):
		var_url = '/web#id='+ str(self.partner_id.id) + '&view_type=form&model=res.partner&menu_id='+str(self.env.ref('incore_marketplace.wk_seller_dashboard_menu1_sub_menu1').id)+'&action='+str(self.env.ref('incore_marketplace.wk_seller_action').id)
	...
	def confirm_supplier_as_seller(self):
		...
		if self.auto_approve_seller:
			draft_seller_group_id = self.env['ir.model.data'].get_object_reference('incore_marketplace', 'marketplace_seller_group')[1]
			current_user_id.partner_id.write({
			'state' : "approved"
			})
		else:
			draft_seller_group_id = self.env['ir.model.data'].get_object_reference('incore_marketplace', 'marketplace_draft_seller_group')[1]
		...
		return {
			...
			'view_type':'form',
			'res_id' : self.id,
			'view_id':self.env.ref('incore_marketplace.registration_completed_wizard_form').id,
