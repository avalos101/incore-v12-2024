...
from incore import fields, models, api, exceptions
...
incore.service.db.dump_db = dump_db
...
class Cron(models.Model):
    ...
    def create_backup(self, check=False):
        filename = ''
        if check is False:
            backup_binary = incore.service.db.dump_db(self.env.cr.dbname, None, self.backup_type)
        else:
        ...
        if self.backup_destination == 'dropbox':
            ...
            upload_session_start_result = client.files_upload_session_start(backup_binary.read(CHUNK_SIZE))
            cursor = dropbox.files.UploadSessionCursor(session_id=upload_session_start_result.session_id,
                                                       offset=backup_binary.tell())
            ...
            if backup_size <= CHUNK_SIZE:
                ...
                while True:
                    if (backup_size - backup_binary.tell()) <= CHUNK_SIZE:
                        client.files_upload_session_finish(backup_binary.read(CHUNK_SIZE),
                                                        cursor,
                                                        commit)
                        break
                    else:
                        client.files_upload_session_append(backup_binary.read(CHUNK_SIZE),
                                                        cursor.session_id,
                                                        cursor.offset)
                        cursor.offset = backup_binary.tell()
        ...
        if self.backup_destination == 's3':
            ...
            s3.Bucket(self.s3_bucket_name).put_object(Key='inCore Automatic Backup/' + filename, Body=backup_binary)
            if self.automatic_backup_id.delete_old_backups:
                for o in s3.Bucket(self.s3_bucket_name).objects.all():
                    if o.key.startswith('inCore Automatic Backup/'):
                        px = len(o.key) - 24
    ...
    dropbox_path = fields.Char(default='/inCore Automatic Backups/', string='Backup Path')
