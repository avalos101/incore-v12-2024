...
class KnowledgeConfigSettings(models.TransientModel):
    ...
    module_cmis_read = fields.Boolean(
        'Attach files from an external DMS into inCore',
        help='Connect inCore with a CMIS compatible server to attach files\n'
             'to an inCore record.\n'
             '- This installs the module cmis_read.'
    )

    module_cmis_write = fields.Boolean(
        'Store attachments in an external DMS instead of the inCore Filestore',
        help='Connect inCore with a CMIS compatible server to store files.\n'
