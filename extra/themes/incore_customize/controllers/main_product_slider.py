...
class WebsiteSale(WebsiteSale):
    ...
    def get_products_content(self, **post):
        ...
        if post.get('tab_id'):
            tab_data = request.env['multitab.configure'].browse(
                int(post.get('tab_id')))
            return request.render("incore_customize.product_slider_content", {'product_collection': tab_data, 'compute_currency': compute_currency, 'limit': post.get('limit') or 0, 'full_width': post.get('full_width')})
        return ''
    ...
    def get_product_brand_slider(self, **post):
        ...
        if post.get('brand-count'):
            brand_ids = request.env['product.brand'].search(
                [('visible_slider', '=', True)] + request.website.website_domain())
            if brand_ids:
                keep = QueryURL('/shop', brand_id=[])
                value.update({'brands': brand_ids,
                              'keep': keep})
        return request.render("incore_customize.brand_slider_content", value)
    ...
    def get_product_latest_p(self, tab_id=0, **post):
        ...
        if tab_id:
            ...
            return request.render("incore_customize.latest_p_content", value)
    ...
    def get_multi_tab_content(self, **post):
        ...
        if post.get('collection_id'):
            ...
            return request.render("incore_customize.s_collection_configure", value)
        return ""
    ...
    def get_brand_multi_tab_content(self, **post):
        ...
        if post.get('collection_id') and post.get('collection_id') != 'false':
            ...
            return {'auto_slider_value': auto_slider_value,
                    ...
                    'slider': request.env.ref("incore_customize.s_brand_collection_configure").render(value)}
        return ""
    ...
    def multi_tab_product_snippet(self, **post):
        ...
        return request.render("incore_customize.product_tab_content", value)
    ...
    def product_slider_collection(self, **post):
        ...
        if post.get('collection_id'):
            ...
            return request.render("incore_customize.product_slider_2_content", value)
    ...
    def get_product_img_gallery(self, **post):
        ...
        if 'product_id' in post and post.get('product_id'):
            ...
            return request.render('incore_customize.product_image_gallery_content', value)
        return ''
