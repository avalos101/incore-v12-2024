...
from incore import fields, http
...
from incore.addons.website_sale.controllers.main import QueryURL
from incore.addons.website_sale.controllers import main
from incore.addons.website.controllers.main import Website
from incore.addons.web_editor.controllers.main import Web_Editor
...
class Web_Editor(Web_Editor):
    ...
    def save_scss(self, url, bundle_xmlid, content):
        ...
        if url != '/incore_customize/static/src/scss/options/colors/color_picker.scss':
        ...
        custom_url = '/incore_customize/static/src/scss/options/colors/color_picker'+str(request.website.id)+'.scss'
...
class Website(Website):
    ...
    def theme_customize(self, enable, disable, get_bundle=True):
        ...
        if enable:
            for i in enable:
                if i.startswith('incore_customize.header_layout_'):
                    if not i.endswith('_css'):
                        templ = i + '_css'
                        views = request.env['ir.ui.view'].search(
                            [('key', 'ilike', 'incore_customize.header_layout_')])
                        for v in views:
                            if v.key.endswith('_css'):
                                set_active([v.key], False)
                        set_active([templ], True)

                if i.startswith('incore_customize.footer_layout_'):
                    if not i.endswith('_css'):
                        templ = i + '_css'
                        views = request.env['ir.ui.view'].search(
                            [('key', 'ilike', 'incore_customize.footer_layout_')])
                        for v in views:
                            if v.key.endswith('_css'):
                                set_active([v.key], False)
                        set_active([templ], True)
...
class WebsiteSale(WebsiteSale):
    ...
    def variant_change_images(self, product_id, **kw):
        ...
        if product_id:
            ...
            if product_rec:
                ...
                ret.update({
                    'value_ret': 1,
                    'main_img': request.env['ir.ui.view'].render_template('incore_customize.main_image_box', values),
                    'sub_img': request.env['ir.ui.view'].render_template('incore_customize.sub_images_box', values),
                    'opt_zoom': int(request.website.viewref('incore_customize.product_picture_magnify').active),
                    'opt_gallery': int(request.website.viewref('incore_customize.product_image_gallery').active)
                })
        return ret
    ...
    def product_brands(self, **post):
        ...
        return request.render(
            'incore_customize.product_brands',
            values)
