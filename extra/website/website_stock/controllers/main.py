...
#################################################################################
from incore import http, tools, api, _
...
class WebsiteSale(WebsiteSale):
    ...
    def cart_update(self, product_id=0, add_qty=0, set_qty=0, **kw):
        if product_id != 0:
            if add_qty == '':
                add_qty = '0'
    ...
    def cart_update_msg(self, product_id, add_qty=None, set_qty=0, **kw):
        result = {'status': 'allow'}
        print("add_qty = " +str(add_qty))
        if add_qty == '':
            add_qty = '0'
        allow_order = request.website.check_if_allowed(int(product_id))
        if allow_order == -1:
    ...
    def shop(self, page=0, category=None, search='', ppg=False, **post):
        ...
        elif config_setting.get('wk_warehouse_type') == 'several':
            stock_location_ids = stock_location_id = config_setting.get('wk_stock_location_ids')
            request.context = dict(request.context, location=stock_location_ids.ids)
    ...
    #    try:
    #        if(product_id == 0):
    #            print("product_id guuuuus= " + str(product_id))
    #    except():
    #        print("hola2" + str(product_id))
