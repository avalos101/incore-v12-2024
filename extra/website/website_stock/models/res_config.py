...
class WebsiteStockConfigSettings(models.Model):
    ...
    wk_warehouse_type = fields.Selection([('all', 'ALL'), ('specific', 'SPECIFIC'), ('several', 'SEVERAL')], string="Default Stock Location", required="1", default="all")

    wk_stock_location = fields.Many2one("stock.location", string="Stock Location", domain="[('usage', '=', 'internal')]")
    
    wk_stock_location_ids = fields.Many2many("stock.location",
       'website_stock_config_settings_stock_location_rel',
       'website_stock_config_id', 'location_id',
