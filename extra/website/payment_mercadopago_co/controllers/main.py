...
class MercadoPagoController(http.Controller):
    _notify_url = '/payment/mercadopago_co/ipn/'
    _return_url = '/payment/mercadopago_co/dpn/'
    _cancel_url = '/payment/mercadopago_co/cancel/'
...
#            return_url = custom.get('return_url', '/')
    ...
    def mercadopago_co_validate_data(self, **post):
...
#       id = identificador-de-la-operación
        ...
        if (not reference and (topic and str(topic) in ["payment"] and op_id) ):
            ...
            reference = request.env["payment.acquirer"].sudo().mercadopago_co_get_reference(payment_id=op_id)

        tx = None
        if reference:
            tx = request.env['payment.transaction'].sudo().search( [('reference', '=', reference)])
            _logger.info('mercadopago_co_validate_data() > payment.transaction founded: %s' % tx.reference)
        ...
        if (tx):
            ...
            res = request.env['payment.transaction'].sudo().form_feedback( post, 'mercadopago_co')

        return res

    @http.route('/payment/mercadopago_co/ipn/', type='json', auth='none')
    def mercadopago_co_ipn(self, **post):
        """ MercadoPago IPN. """
        ...
        if (params and ('topic' in params or 'type' in params) and ('id' in params or 'data.id' in params)):
            self.mercadopago_co_validate_data( **params )
        else:
            self.mercadopago_co_validate_data(**post)
        return ''

    @http.route('/payment/mercadopago_co/dpn/', type='http', auth="none")
    def mercadopago_co_dpn(self, **post):
        """ MercadoPago DPN """
    ...
    @http.route('/payment/mercadopago_co/cancel', type='http', auth="none")
    def mercadopago_co_cancel(self, **post):
