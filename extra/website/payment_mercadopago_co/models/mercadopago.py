...
from incore.addons.payment_mercadopago_co.controllers.main import MercadoPagoController
from incore import osv, fields, models, api
...
from incore.addons.payment_mercadopago_co.mercadopago import mercadopago

class AcquirerMercadopago(models.Model):
    _inherit = 'payment.acquirer'

    def _get_mercadopago_co_urls(self, environment):
        """ MercadoPago URLS """
        if environment == 'prod':
            return {
                'mercadopago_form_url': 'https://www.mercadopago.com/mco/checkout/pay',
                'mercadopago_rest_url': 'https://api.mercadolibre.com/oauth/token',
            }
        else:
            return {
                'mercadopago_form_url': 'https://sandbox.mercadopago.com/mco/checkout/pay',
    ...
    def _get_providers(self, context=None):
        ...
        providers.append(['mercadopago_co', 'MercadoPago Colombia'])
        return providers
    ...
    provider = fields.Selection(selection_add=[('mercadopago_co', 'MercadoPago Colombia')])
    ...
    def _migrate_mercadopago_co_account(self, context=None):
        """ COMPLETE ME """
        ...
        #for (company_id, company_mercadopago_account) in res:
            if company_mercadopago_account:
                company_mercadopago_ids = self.search([('company_id', '=', company_id), ('provider', '=', 'mercadopago_co')], limit=1, context=context)
                if company_mercadopago_ids:
                    ...
                    mercadopago_view = self.env['ir.model.data'].get_object(cr, uid, 'payment_mercadopago_co', 'mercadopago_co_acquirer_button')
                    self.create({
                        'name': 'MercadoPago',
    ...
    def mercadopago_co_compute_fees(self, amount, currency_id, country_id):
    ...
    def mercadopago_co_dateformat(self, date):
    ...
    @api.multi
    def mercadopago_co_form_generate_values(self, values):
...
#	            },
	            ...
	            "expires": True,
	            "expiration_date_from": self.mercadopago_co_dateformat( datetime.datetime.now(tzlocal())-datetime.timedelta(days=1) ),
	            "expiration_date_to": self.mercadopago_co_dateformat( datetime.datetime.now(tzlocal())+datetime.timedelta(days=31) )
                }
    ...
    def mercadopago_co_get_form_action_url(self):
        return self._get_mercadopago_co_urls( self.environment)['mercadopago_form_url']

    def _mercadopago_co_s2s_get_access_token(self, ids, context=None):
        """
        ...
        for acquirer in self.browse( ids, context=context):
            tx_url = self._get_mercadopago_co_urls( acquirer.environment)['mercadopago_rest_url']
    ...
    @api.model
    def mercadopago_co_get_reference(self, payment_id=None ):
        reference = None
        if (not payment_id):
            return reference
        mps = self.search([('provider','=','mercadopago_co'),('mercadopago_client_id','!=',False),('mercadopago_secret_key','!=',False)])
        for mp in mps:
            data = mp._mercadopago_co_get_data(payment_id=payment_id)
            if (data and ('external_reference' in data) ):
                return data['external_reference']
        return reference


    def _mercadopago_co_get_data(self, payment_id=None, reference=None ):
        data = None
...
class TxMercadoPago(models.Model):
    ...
    def action_mercadopago_co_check_status( self ):
        data = {}
        _logger.info("action_mercadopago_co_check_status")
        for tx in self:
            ...
            try:
                data = tx.acquirer_id._mercadopago_co_get_data(reference=acquirer_reference)
                #_logger.info(data)
                if (data):
                    tx._mercadopago_co_form_validate(dict(data))
    ...
    # --------------------------------------------------
    @api.model
    def _mercadopago_co_form_get_tx_from_data(self, data, context=None):
#        reference, txn_id = data.get('external_reference'), data.get('txn_id')
    ...
    def _mercadopago_co_form_get_invalid_parameters(self, data):
        invalid_parameters = []
...
#charged_back (estado terminal) 	Se realizó un contracargo en la tarjeta de crédito.
    ...
    def _mercadopago_co_form_validate(self, data):
        #IPN style
        f_data = data
        ...
        if (topic in ["payment"] and payment_id and self.acquirer_id):
            ...
            res = self.acquirer_id._mercadopago_co_get_data(payment_id=payment_id)
            if (res):
                f_data.update( res )
            external_reference = f_data.get('external_reference')
        elif (external_reference):
            #DPN based on payment id, preferred...
            res = self.acquirer_id._mercadopago_co_get_data(reference=external_reference)
            if (res):
                f_data.update( res )
        ...
        _logger.info("_mercadopago_co_form_validate: external_reference: " +str(external_reference))
    ...
    def _mercadopago_co_try_url(self, request, tries=3, context=None):
        ...
        the communication as failed.
         ...
         .. warning::
            ...
            Experimental code. You should not use it before inCore v8 official
            release.
        """
        done, res = False, None
        ...
        if not res:
            pass
            # raise incore.exceptions.
        result = res.read()
        res.close()
        return result

    def _mercadopago_co_s2s_send(self, values, cc_values, context=None):
        """
         .. versionadded:: pre-v8 saas-3
         .. warning::
        ...
        result = self._mercadopago_co_try_url(request, tries=3, context=context)
        return (tx_id, result)

    def _mercadopago_co_s2s_get_invalid_parameters(self, tx, data, context=None):
        """
         .. versionadded:: pre-v8 saas-3
         .. warning::
    ...
    def _mercadopago_co_s2s_validate(self, tx, data, context=None):
        """
         .. versionadded:: pre-v8 saas-3
         .. warning::
    ...
    def _mercadopago_co_s2s_get_tx_status(self, tx, context=None):
        """
         .. versionadded:: pre-v8 saas-3
         .. warning::
        ...
        data = self._mercadopago_co_try_url(request, tries=3, context=context)
