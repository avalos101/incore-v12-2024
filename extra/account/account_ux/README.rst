...
#. Make origin always visible on invoices. By default incore only make it visible when it has a value. The issue is that a user can delete the value but can't restore it again. We also think is a good idea to make it editable in case you want to link a manual invoice to, for eg, a sale order
...
#. This replace original incore wizard for changing currency on an invoice with serveral improvements:
