...
class AccountInvoice(models.Model):
    ...
    def _send_edi_email(self):
        for rec in self:
            ...
            ctx = dict(active_model='account.invoice')
            ...
            for invoice_line_id in rec.invoice_line_ids:
                if invoice_line_id.account_id:
                    ...
                    if invoice_line_id.product_id.uom_id.edi_unit_measure_id.id:
                        ...
                        # If the setting is used in the inCore unit of measure, then this field is not required
            ...
            if type_edi_document != 'none':
                ...
                if rec.currency_id and rec.company_id and rec.currency_id != rec.company_id.currency_id:
                    ...
                    if company_currency_search and invoice_currency_search:
                        ...
                        raise UserError(_("A currency type in inCore does not correspond to any DIAN currency type"))
    ...
    def validate_dian_generic(self, is_test):
        for rec in self:
            ...
            try:
                if rec.state == 'draft':
                    raise UserError(_("The invoice must first be validated in inCore, before being sent to the DIAN."))
    ...
    def status_document(self):
        for rec in self:
            ...
            try:
                # This line ensures that the electronic fields of the invoice are updated in inCore, before the request
                requests_data = rec.get_json_request()
    ...
    def status_document_log(self):
        for rec in self:
            ...
            try:
                # This line ensures that the electronic fields of the invoice are updated in inCore,
                # before request
                requests_data = rec.get_json_request()
                _logger.debug('Customer data: %s', requests_data)
