...
class TestMicropay(TestPointOfSaleCommon):
    ...
    def test_micropay_backend(self):
        ...
        * Cashier scanned buyer's QR and upload it to incore server,
        incore server sends information to wechat servers and wait for response with result.

        * Once user authorize the payment, incore receives result syncroniosly from
        previously sent request.

        * inCore sends result to POS via longpolling.
