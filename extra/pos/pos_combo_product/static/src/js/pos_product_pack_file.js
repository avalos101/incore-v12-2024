incore.define('pos_combo_product.pos_product_pack_file', function (require) {
"use strict";
            ...
            if(pack_product[i].id==pack_product_id && self.product.is_combo && (pack_product[i].combo_product_id).length>0 )
            {
                for(var j=0;j<(pack_product[i].combo_product_id).length;j++){ 
                    ...
                    for(var k=0;k<pack_products.length;k++){
                        ...
                        var product_val={'display_name':pack_products[k].product_id[2],'uom_id':pack_products[k].uom_id[2],'price':pack_products[k].price, 'qty':pack_products[k].product_quantity};
