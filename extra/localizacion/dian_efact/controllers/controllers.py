...
class dianefact(http.Controller):
    ...
    def get_invoice_ordered(self, **kw):
        ...
        query = "select qr_image, number, journal_id from account_invoice where id = "+str(invoice_id)
        request.cr.execute(query)    
        ...
        response = {"number":account_invoice['number'],"qr_image":account_invoice['qr_image'],"journal_name":account_journal['name']}
