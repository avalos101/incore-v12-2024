...
from incore.exceptions import Warning, UserError
...
class account_invoice(models.Model):
    ...
    def invoice_validate(self):
        ...
        for invoice in self:
            if not invoice.company_id.dian_xml_client_path:
                raise Warning(_("Campos faltantes: Debe llenar el campo NIT/Usuario en Compañia!!"))
        ...
        if(eDocumentType == "NCR"):
            for invoice in self:
                ...
                base_url += '/web#id=%d&view_type=form&model=%s' % (
                    self.id, self._name)
                self.qr_image = self.generate_QR(base_url)
            ...
            for invoice in self:
                ...
                #with open('/incore_diancol/custom/addons/dian_efact/log.json', 'w') as outfile:
...
#
    ...
    def validate_dian_data(self, dian_data):
        ...
        if(not dian_data["emisor"]["correo_electronico"]):
            errors = str(errors) + \
                str("- Correo electrónico del emisor.") + str("\n")
        if(not dian_data["emisor"]["telefono"]):
            errors = str(errors) + str("- Télefono del emisor.") + str("\n")
        if(not dian_data["receptor"]["correo_electronico"]):
            errors = str(errors) + str("- Télefono del receptor.") + str("\n")
        ...
        if(errors != ""):
            errors = str("Completar los siguientes campos: ") + \
                str("\n") + errors
        ...
        try:
            ...
            if not os.path.exists(xmlPath+'/XMLcertificates/'+xmlClientPath):
                os.makedirs(xmlPath+'/XMLcertificates/' +
                            xmlClientPath, mode=0o777)
            os.system("sudo su")
            os.system("sudo chmod -R 777"+xmlPath)
            # Permisos no se estan asignando correctamente a la carpeta del usuario registrado 
