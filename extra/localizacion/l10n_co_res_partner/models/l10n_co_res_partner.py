...
from incore.osv.expression import get_unaccent_wrapper
import re
import logging
...
class PartnerInfoExtended(models.Model):
    ...
    def onChangeName(self):
        ...
        name has no value. Reason: In certain forms of incore it is still
    ...
    """
    Re-emplaza toda la funcion _name_search para agregar el campo xidentification en el buscador
    """
    @api.model
    def _name_search(self, name, args=None, operator='ilike', limit=100, name_get_uid=None):
        ...
        if name and operator in ('=', 'ilike', '=ilike', 'like', '=like'):
            ...
            query = """SELECT res_partner.id
                         FROM {from_str}
                      {where} ({xidentification} {operator} {percent}
                    ...
                    """.format(from_str=from_str,
                               where=where_str,
                               operator=operator,
                               xidentification=unaccent('res_partner.xidentification'),
            ...
            if partner_ids:
                ...
                return []
        return super(PartnerInfoExtended, self)._name_search(name, args, operator=operator, limit=limit, name_get_uid=name_get_uid)
