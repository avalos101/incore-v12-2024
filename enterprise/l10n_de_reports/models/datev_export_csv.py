...
class DatevExportCSV(models.AbstractModel):
    ...
    def _find_partner_account(self, account, partner):
        if (account.internal_type in ('receivable', 'payable') and partner):
            ...
            # Which has only a number. To differenciate between the two, if a partner in incore
            # explicitely has a receivable/payable account set, we use that account, othewise
            # We assume it is not an important partner and his datev virtual id will be his incore id
