...
DEFAULT_ENDPOINT = 'https://ocn.incore.co'
...
class ResConfigSettings(models.TransientModel):
    ...
    def _get_endpoint(self):
        return self.env['ir.config_parameter'].sudo().get_param('incore_ocn.endpoint', DEFAULT_ENDPOINT)

    @api.model
    def create(self, vals):
        ir_params = self.env['ir.config_parameter'].sudo()
        if vals.get('ocn_push_notification') and not ir_params.get_param('incore_ocn.project_id'):
            ...
            ir_params.set_param('incore_ocn.project_id', project_id)
