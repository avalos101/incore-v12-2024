...
class account_bank_reconciliation_report(models.AbstractModel):
    ...
    def _get_bank_rec_report_data(self, options, line_id=None):
        ...
        rslt['incore_balance'] = sum([line.amount_currency if rslt['use_foreign_currency'] else line.balance for line in lines_already_accounted])
    ...
    def _get_lines(self, options, line_id=None):
        ...
        computed_stmt_balance = report_data['incore_balance'] + outstanding_plus_tot + outstanding_minus_tot + unrec_tot
        ...
        lines.append(self._add_title_line(options, gl_title % accounts_string, level=1, amount=report_data['incore_balance'], date=options['date']['date']))
...
If non-zero, it could be due to
  1) some bank statements being not yet encoded into inCore
