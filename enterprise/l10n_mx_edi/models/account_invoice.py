...
from incore import _, api, fields, models, tools
from incore.tools.xml_utils import _check_with_xsd
from incore.tools import DEFAULT_SERVER_TIME_FORMAT
from incore.tools import float_round
from incore.exceptions import UserError
from incore.tools.float_utils import float_repr
...
class AccountInvoice(models.Model):
    ...
    def l10n_mx_edi_update_pac_status(self):
        '''Synchronize both systems: inCore & PAC if the invoices need to be signed or cancelled.
        '''
        for record in self:
            if record.l10n_mx_edi_pac_status in ('to_sign', 'retry'):
                record._l10n_mx_edi_retry()
