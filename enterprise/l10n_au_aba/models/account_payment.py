...
class AccountPayment(models.Model):
    ...
    def _check_bank_account(self):
        self.ensure_one()
        if self.payment_method_id == self.env.ref('l10n_au_aba.account_payment_method_aba_ct'):
            ...
            if bank_acc.acc_type != 'aba' or not bank_acc.aba_bsb:
                raise ValidationError(_("Journal '%s' requires a proper ABA account. Please configure it first.") % self.journal_id.name)

            if not self.journal_id.aba_user_spec or not self.journal_id.aba_fic or not self.journal_id.aba_user_number:
                raise ValidationError(_("Please fill in the ABA data of account %s (journal %s) before using it to generate ABA payments.")
                    % (bank_acc.acc_number, self.journal_id.name))

    @api.constrains('payment_method_id', 'partner_bank_account_id')
    def _check_partner_bank_account(self):
        self.ensure_one()
        if self.payment_method_id == self.env.ref('l10n_au_aba.account_payment_method_aba_ct'):
            if self.partner_bank_account_id.acc_type != 'aba' or not self.partner_bank_account_id.aba_bsb:
