...
class ProviderAccount(models.Model):
    ...
    def log_message(self, message):
        ...
        # This call can also fail and if this is the case we do not want to prevent the user from deleting the object in incore.
    ...
    def _get_favorite_institutions(self, country):
        resp_json = {}
        try:
            resp = requests.post(url='https://onlinesync.incore.co/onlinesync/search/favorite', data={'country': country, 'provider': json.dumps(self._get_available_providers())}, timeout=60)
            resp_json = resp.json()
    ...
    def get_institutions(self, searchString, country):
        ...
        resp_json = {}
        try:
            resp = requests.post(url='https://onlinesync.incore.co/onlinesync/search/', data={'query': searchString, 'country': country, 'provider': json.dumps(self._get_available_providers())}, timeout=60)
            resp_json = resp.json()
        except requests.exceptions.Timeout:
            raise UserError(_('Timeout: the server did not reply within 60s'))
        except (ValueError, requests.exceptions.ConnectionError):
