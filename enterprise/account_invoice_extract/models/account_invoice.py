...
from incore.addons.iap import jsonrpc
from incore import api, exceptions, fields, models, _
...
PARTNER_REMOTE_URL = 'https://partner-autocomplete.incore.co/iap/partner_autocomplete'
                ...
                if record.extract_state == "no_extract_requested":
                    ...
                    if attachments:
                        endpoint = self.env['ir.config_parameter'].sudo().get_param(
                            'account_invoice_extract_endpoint', 'https://iap-extract.incore.co') + '/iap/invoice_extract/parse'
                        params = {
    ...
    def invoice_validate(self):
        ...
        for record in self:
            ...
            if record.extract_state == 'waiting_validation':
                endpoint = self.env['ir.config_parameter'].sudo().get_param(
                    'account_invoice_extract_endpoint', 'https://iap-extract.incore.co')  + '/iap/invoice_extract/validate'
                values = {
    ...
    def check_status(self):
        ...
        for record in self:
            if record.extract_state not in ["waiting_extraction", "extract_not_ready"]:
                continue
            endpoint = self.env['ir.config_parameter'].sudo().get_param(
                'account_invoice_extract_endpoint', 'https://iap-extract.incore.co')  + '/iap/invoice_extract/get_result'
            params = {
                'version': CLIENT_OCR_VERSION,
                'document_id': record.extract_remoteid
            }
