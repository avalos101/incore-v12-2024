...
from incore.tools import frozendict, classproperty, StackMap, pycompat
from incore.exceptions import CacheMiss
...
def constrains(*args):
    ...
    Should raise :class:`~incore.exceptions.ValidationError` if the
    validation failed.
...
def one(method):
    """ Decorate a record-style method where ``self`` is expected to be a
        ...
        .. deprecated:: 9.0
            ...
            is a single record with :meth:`~incore.models.Model.ensure_one`.
    """
...
from incore.exceptions import UserError, AccessError, MissingError
