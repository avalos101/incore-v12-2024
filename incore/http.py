...
# *broken*
NO_POSTMORTEM = (incore.osv.orm.except_orm,
                 incore.exceptions.AccessError,
                 incore.exceptions.ValidationError,
                 incore.exceptions.MissingError,
                 incore.exceptions.AccessDenied,
                 incore.exceptions.Warning,
                 incore.exceptions.RedirectWarning)
    ...
    try:
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            if rpc_request and rpc_response_flag:
                incore.netsvc.log(rpc_request, logging.DEBUG, '%s.%s' % (service_name, method), replace_request_password(params))

        threading.current_thread().uid = None
        threading.current_thread().dbname = None
        if service_name == 'common':
            dispatch = incore.service.common.dispatch
        elif service_name == 'db':
            dispatch = incore.service.db.dispatch
        elif service_name == 'object':
            dispatch = incore.service.model.dispatch
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            if rpc_response_flag:
                incore.netsvc.log(rpc_response, logging.DEBUG, logline, result)
            else:
                incore.netsvc.log(rpc_request, logging.DEBUG, logline, replace_request_password(params), depth=1)

        return result
    except NO_POSTMORTEM:
        raise
    except incore.exceptions.DeferredException as e:
        _logger.exception(incore.tools.exception_to_unicode(e))
        incore.tools.debugger.post_mortem(incore.tools.config, e.traceback)
        raise
    except Exception as e:
        _logger.exception(incore.tools.exception_to_unicode(e))
        incore.tools.debugger.post_mortem(incore.tools.config, sys.exc_info())
        raise
...
class WebRequest(object):
    """ Parent class for all inCore Web request types, mostly deals with
    ...
    def __init__(self, httprequest):
        ...
        # dispatching phase in incore.service.wsgi_server.application
        if self.db:
    ...
    @property
    def cr(self):
        """ :class:`~incore.sql_db.Cursor` initialized for the current method call.
    ...
    @property
    def env(self):
        """ The :class:`~incore.api.Environment` bound to current request. """
        if self._env is None:
            self._env = incore.api.Environment(self.cr, self.uid, self.context)
        return self._env
    ...
    @lazy_property
    def session(self):
        """ :class:`inCoreSession` holding the HTTP session data for the
    ...
    def _handle_exception(self, exception):
        ...
        if not isinstance(exception, NO_POSTMORTEM) \
            ...
            incore.tools.debugger.post_mortem(
                incore.tools.config, sys.exc_info())
    ...
    def registry(self):
        ...
        .. deprecated:: 8.0
            ...
            use :attr:`.env`
        """
        return incore.registry(self.db)
...
def route(route=None, **kw):
    ...
    .. admonition:: CSRF Protection
        ...
        inCore implements token-based `CSRF protection
        ...
        * if the form is generated in Python, a csrf token is
          ...
          <incore.http.WebRequest.csrf_token`, the
          :data:`~incore.http.request` object is available by default
        ...
        * if the endpoint can be called by external parties (not from
          inCore) as e.g. it is a REST API or a `webhook
    ...
    * the handler method's result is returned as JSON-RPC ``result`` and
    ...
    def _handle_exception(self, exception):
        ...
        try:
            ...
            if not isinstance(exception, (incore.exceptions.Warning, SessionExpiredException,
                                          incore.exceptions.except_orm, werkzeug.exceptions.NotFound)):
                _logger.exception("Exception during JSON request handling.")
            error = {
                    'code': 200,
                    'message': "inCore Server Error",
            ...
            if isinstance(exception, AuthenticationError):
                error['code'] = 100
                error['message'] = "inCore Session Invalid"
            if isinstance(exception, SessionExpiredException):
                error['code'] = 100
                error['message'] = "inCore Session Expired"
...
def serialize_exception(e):
    ...
    elif isinstance(e, incore.exceptions.Warning):
    ...
    elif isinstance(e, incore.exceptions.ValidationError):
        tmp["exception_type"] = "validation_error"
    elif isinstance(e, incore.exceptions.except_orm):
...
inCore URLs are CSRF-protected by default (when accessed with unsafe
HTTP methods). See
https://www.incore.co/documentation/12.0/reference/http.html#csrf for
more details.

* if this endpoint is accessed through inCore via py-QWeb form, embed a CSRF
...
class ControllerType(type):
    def __init__(cls, name, bases, attrs):
        ...
        if not class_path[:2] == ["incore", "addons"]:
            module = ""
        else:
...
def routing_map(modules, nodb_only, converters=None):
    ...
    def get_subclasses(klass):
        def valid(c):
            return c.__module__.startswith('incore.addons.') and c.__module__.split(".")[2] in modules
...
class inCoreSession(werkzeug.contrib.sessions.Session):
    def __init__(self, *args, **kwargs):
        self.inited = False
        self.modified = False
        self.rotate = False
        super(inCoreSession, self).__init__(*args, **kwargs)
        self.inited = True
    ...
    def authenticate(self, db, login=None, password=None, uid=None):
        ...
        if uid is None:
            ...
            uid = incore.registry(db)['res.users'].authenticate(db, login, password, env)
        else:
    ...
    def check_security(self):
        ...
        env = incore.api.Environment(request.cr, self.uid, self.context)
    ...
    def _fix_lang(self, context):
        """ inCore provides languages which may not make sense and/or may not
...
class Response(werkzeug.wrappers.Response):
    ...
    def render(self):
        ...
        """
        env = request.env(user=self.uid or request.uid or incore.SUPERUSER_ID)
...
class Root(object):
    """Root WSGI application for the inCore Web Client.
    """
    def __init__(self):
        self._loaded = False
    ...
    def session_store(self):
        # Setup http sessions
        path = incore.tools.config.session_dir
        _logger.debug('HTTP sessions stored in: %s', path)
        return werkzeug.contrib.sessions.FilesystemSessionStore(
            path, session_class=inCoreSession, renew_missing=True)

    @lazy_property
    def nodb_routing_map(self):
        _logger.info("Generating nondb routing")
        return routing_map([''] + incore.conf.server_wide_modules, True)
    ...
    def load_addons(self):
        ...
        statics = {}
        for addons_path in incore.modules.module.ad_paths:
    ...
    def setup_session(self, httprequest):
        ...
        if not sid:
            sid =  httprequest.headers.get("X-Incore-Session-Id")
        if not sid:
    ...
    def dispatch(self, environ, start_response):
        ...
        try:
            ...
            with request:
                ...
                if db:
                    try:
                        incore.registry(db).check_signaling()
                        with incore.tools.mute_logger('incore.sql_db'):
...
def db_list(force=False, httprequest=None):
    dbs = incore.service.db.list_dbs(force)
...
def db_filter(dbs, httprequest=None):
    ...
    if d == "www" and r:
        d = r.partition('.')[0]
    if incore.tools.config['dbfilter']:
        d, h = re.escape(d), re.escape(h)
        r = incore.tools.config['dbfilter'].replace('%h', h).replace('%d', d)
        dbs = [i for i in dbs if re.match(r, i)]
    elif incore.tools.config['db_name']:
        # In case --db-filter is not provided and --database is passed, inCore will
        # use the value of --database as a comma seperated list of exposed databases.
        exposed_dbs = set(db.strip() for db in incore.tools.config['db_name'].split(','))
...
def send_file(filepath_or_fp, mimetype=None, as_attachment=False, filename=None, mtime=None,
    ...
    if isinstance(mtime, str):
        try:
            server_format = incore.tools.misc.DEFAULT_SERVER_DATETIME_FORMAT
...
def content_disposition(filename):
    filename = incore.tools.ustr(filename)
