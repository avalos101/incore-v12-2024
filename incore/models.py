...
class MetaModel(api.Meta):
    ...
    def _get_addon_name(self, full_name):
        # The (inCore) module name can be in the ``incore.addons`` namespace
        # or not. For instance, module ``sale`` can be imported as
        # ``incore.addons.sale`` (the right way) or ``sale`` (for backward
        # compatibility).
        module_parts = full_name.split('.')
        if len(module_parts) > 2 and module_parts[:2] == ['incore', 'addons']:
...
class BaseModel(MetaModel('DummyModel', (object,), {'_register': False})):
    """ Base class for inCore models.

    inCore models are created by inheriting:
    ...
    *   :class:`AbstractModel` for abstract super classes meant to be shared by
    ...
    def compute_concurrency_field(self):
        for record in self:
            record[self.CONCURRENCY_CHECK_FIELD] = incore.fields.Datetime.now()

    @api.depends('create_date', 'write_date')
    def compute_concurrency_field_with_access(self):
        for record in self:
            record[self.CONCURRENCY_CHECK_FIELD] = \
                record.write_date or record.create_date or incore.fields.Datetime.now()
    ...
    def _add_fake_fields(self, fields):
        from incore.fields import Char, Integer
    ...
    def write(self, vals):
        ...
        * For numeric fields (:class:`~incore.fields.Integer`,
          :class:`~incore.fields.Float`) the value should be of the
          corresponding type
        * For :class:`~incore.fields.Boolean`, the value should be a
          :class:`python:bool`
        * For :class:`~incore.fields.Selection`, the value should match the
          selection values (generally :class:`python:str`, sometimes
          :class:`python:int`)
        * For :class:`~incore.fields.Many2one`, the value should be the
        ...
        * Other non-relational fields use a string for value
          ...
          .. danger::
              ...
              :class:`~incore.fields.Date` and
              :class:`~incore.fields.Datetime` fields use strings as values
              (written and read) rather than :class:`~python:datetime.date` or
              :class:`~python:datetime.datetime`. These date strings are
              UTC-only and formatted according to
              :const:`incore.tools.misc.DEFAULT_SERVER_DATE_FORMAT` and
              :const:`incore.tools.misc.DEFAULT_SERVER_DATETIME_FORMAT`
        * .. _incore/models/relationals/format:

          :class:`~incore.fields.One2many` and
          :class:`~incore.fields.Many2many` use a special "commands" format to
          ...
          ``(3, id, _)``
              ...
              :class:`~incore.fields.One2many`. Can not be used in
              :meth:`~.create`.
          ``(4, id, _)``
              adds an existing record of id ``id`` to the set. Can not be
              used on :class:`~incore.fields.One2many`.
          ``(5, _, _)``
    ...
    def _where_calc(self, domain, active_test=True):
        """Computes the WHERE clause needed to implement an inCore domain.
    ...
    def with_env(self, env):
        ...
        :type env: :class:`~incore.api.Environment`
        """
...
class Model(AbstractModel):
    """ Main super-class for regular database-persisted inCore models.

    inCore models are created by inheriting from this class::
