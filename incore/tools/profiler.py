...
class _LogTracer(object):
    ...
    def tracer(self, frame, event, arg):
        ...
        if isinstance(in_self, (incore.sql_db.Cursor, incore.sql_db.TestCursor, incore.sql_db.LazyCursor)):
            return self.tracer
...
def profile(method=None, whitelist=None, blacklist=(None,), files=None,
    ...
    """
        ...
        all methods for all incore models by applying the optional filters.
        ...
        :param list blacklist: list model names to remove from the log
                        (Default: remove non incore model from the log: [None])
        ...
        .. code-block:: python
          ...
          class SaleOrder(models.Model):
            ...
            ...
            @api.multi
            @profile()                  # log all methods for all incore models
            def unlink(self):
            ...
            @profile(whitelist=['sale.order', 'ir.model.data'])
            def action_quotation_send(self):
            ...
            @profile(files=['/home/incore/incore/incore/addons/sale/models/sale.py'])
            def write(self):
            ...
    ...
    def _incoreProfile(method, *args, **kwargs):
    ...
    if not method:
        return lambda method: decorator(_incoreProfile, method)

    wrapper = decorator(_incoreProfile, method)
    return wrapper
