...
def _get_idref(self, env, model_str, idref):
    idref2 = dict(idref,
                  ...
                  version=incore.release.major_version,
                  ref=self.id_get,
                  pytz=pytz)
    if model_str:
    ...
    if node.tag in ('field','value'):
        ...
        if a_eval:
            ...
            try:
                ...
                logging.getLogger('incore.tools.convert.init').error(
        ...
        return incore.api.call_kw(model, method, args, {})
    elif node.tag == "test":
        return node.text
...
form: module.record_id""" % (xml_id,)
    ...
    def parse(self, de, mode=None):
        roots = ['incore','data','incore']
        if de.tag not in roots:
            raise Exception("Root xml tag must be <incore>, <incore> or <data>.")
        for rec in de:
            if rec.tag in roots:
                self.parse(rec, mode)
    ...
    def __init__(self, cr, module, idref, mode, report=None, noupdate=False, xml_filename=None):
        self.mode = mode
        self.module = module
        self.env = incore.api.Environment(cr, SUPERUSER_ID, {})
        self.cr = cr
