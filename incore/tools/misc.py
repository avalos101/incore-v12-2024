...
"""
Miscellaneous tools used by inCore.
"""
...
from incore.loglevels import get_encodings, ustr, exception_to_unicode     # noqa
...
def exec_pg_environ():
    ...
    """
    env = os.environ.copy()
    if incore.tools.config['db_host']:
        env['PGHOST'] = incore.tools.config['db_host']
    if incore.tools.config['db_port']:
        env['PGPORT'] = str(incore.tools.config['db_port'])
    if incore.tools.config['db_user']:
        env['PGUSER'] = incore.tools.config['db_user']
    if incore.tools.config['db_password']:
        env['PGPASSWORD'] = incore.tools.config['db_password']
    return env
...
def file_open(name, mode="r", subdir='addons', pathinfo=False):
    """Open a file from the inCore root, using a subdir folder.
    ...
    """
    import incore.modules as addons
    ...
    if os.path.isabs(name):
        ...
        for root in adps + [rtp]:
            ...
            # It is outside the inCore root: skip zipfile lookup.
...
def flatten(list):
    ...
    Author: Christophe Simonis (christophe@incore.co)
...
def scan_languages():
    """ Returns all languages supported by inCore for translation

    :returns: a list of (lang_code, lang_name) pairs
    :rtype: [(str, unicode)]
    """
    csvpath = incore.modules.module.get_resource_path('base', 'data', 'res.lang.csv')
    try:
...
class UnquoteEvalContext(defaultdict):
    """Defaultdict-based evaluation context that returns 
       ...
       the evaluation.
       Mostly useful for evaluating inCore domains/contexts that
...
class mute_logger(object):
    ...
    Can be used as context manager or decorator.
        ...
        @mute_logger('incore.plic.ploc')
        def do_stuff():
            blahblah()

        with mute_logger('incore.foo.bar'):
            do_suff()
...
def format_date(env, value, lang_code=False, date_format=False):
    ...
    if isinstance(value, pycompat.string_types):
        ...
        if len(value) > DATE_LENGTH:
            ...
            value = incore.fields.Datetime.from_string(value)
            value = incore.fields.Datetime.context_timestamp(env['res.lang'], value)
        else:
            value = incore.fields.Datetime.from_string(value)
