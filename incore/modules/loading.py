...
import incore
import incore.modules.db
import incore.modules.graph
import incore.modules.migration
import incore.modules.registry
import incore.tools as tools

from incore import api, SUPERUSER_ID
from incore.modules.module import adapt_version, initialize_sys_path, load_incore_module

_logger = logging.getLogger(__name__)
_test_logger = logging.getLogger('incore.tests')
...
def force_demo(cr):
    ...
    """
    graph = incore.modules.graph.Graph()
...
def load_module_graph(cr, graph, status=None, perform_checks=True,
    ...
    def load_test(idref, mode):
        ...
        try:
            ...
            incore.registry(cr.dbname).clear_caches()
    ...
    processed_modules = []
    loaded_modules = []
    registry = incore.registry(cr.dbname)
    migrations = incore.modules.migration.MigrationManager(cr, graph)
    ...
    for index, package in enumerate(graph, 1):
        ...
        load_incore_module(package.name)

        new_install = package.state == 'to install'
        if new_install:
            py_module = sys.modules['incore.addons.%s' % (module_name,)]
        ...
        if needs_update:
            ...
            overwrite = incore.tools.config["overwrite_existing_translations"]
            ...
            if demo_loaded:
                ...
                if tools.config.options['test_enable']:
                    ...
                    report.record_result(incore.modules.module.run_unit_tests(module_name, cr.dbname))
    ...
    _logger.log(25, "%s modules loaded in %.2fs, %s queries", len(graph), time.time() - t0, incore.sql_db.sql_counter - t0_sql)
...
def load_modules(db, force_demo=False, status=None, update_module=False):
    ...
    with db.cursor() as cr:
        if not incore.modules.db.is_initialized(cr):
        ...
        # incore.modules.registry.Registry.new().
        registry = incore.registry(cr.dbname)
        ...
        if update_module:
            ...
            if modules_to_remove:
                ...
                for pkg in pkgs:
                    ...
                    if uninstall_hook:
                        py_module = sys.modules['incore.addons.%s' % (pkg.name,)]
                ...
                registry = incore.modules.registry.Registry.new(
