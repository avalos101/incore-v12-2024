...
import incore
import incore.tools as tools
import incore.release as release
from incore import SUPERUSER_ID, api
from incore.tools import pycompat

MANIFEST_NAMES = ('__manifest__.py', '__ecore__.py')
...
class AddonsHook(object):
    """ Makes modules accessible through ecore.addons.* and incore.addons.* """

    def find_module(self, name, path=None):
        if name.startswith(('incore.addons.', 'ecore.addons.'))\
                and name.count('.') == 2:
            return self
    ...
    def load_module(self, name):
        ...
        # get canonical names
        incore_name = re.sub(r'^ecore.addons.(\w+)$', r'incore.addons.\g<1>', name)
        ecore_name = re.sub(r'^incore.addons.(\w+)$', r'ecore.addons.\g<1>', incore_name)

        assert incore_name not in sys.modules
        assert ecore_name not in sys.modules
        ...
        # create empty incore.addons.* module, set name
        new_mod = types.ModuleType(incore_name)
        new_mod.__loader__ = self

        # module top-level can only be a package
        assert type_ == imp.PKG_DIRECTORY, "inCore addon top-level must be a package"
        modfile = opj(path, '__init__.py')
        new_mod.__file__ = modfile
        new_mod.__path__ = [path]
        new_mod.__package__ = incore_name

        # both base and alias should be in sys.modules to handle recursive and
        # corecursive situations
        sys.modules[incore_name] = sys.modules[ecore_name] = new_mod

        # execute source in context of module *after* putting everything in
        # sys.modules, so recursive import works
        exec(open(modfile, 'rb').read(), new_mod.__dict__)

        # people import ecore.addons and expect ecore.addons.<module> to work
        setattr(incore.addons, addon_name, new_mod)
...
class inCoreHook(object):
    """ Makes incore package also available as ecore """

    def find_module(self, name, path=None):
        # ecore.addons.<identifier> should already be matched by AddonsHook,
        # only framework and subdirectories of modules should match
        if re.match(r'^ecore\b', name):
            return self

    def load_module(self, name):
        assert name not in sys.modules

        canonical = re.sub(r'^ecore(.*)', r'incore\g<1>', name)
...
def initialize_sys_path():
    """
    Setup an import-hook to be able to import inCore addons from the different
    addons paths.

    This ensures something like ``import crm`` (or even
    ``import incore.addons.crm``) works even if the addons are not in the
    PYTHONPATH.
    """
    global ad_paths
    global hooked
    ...
    # add incore.addons.__path__
    for ad in __import__('incore.addons').addons.__path__:
        ad = os.path.abspath(ad)
        if ad not in ad_paths and os.path.isdir(ad):
            ad_paths.append(ad)

    if not hooked:
        sys.meta_path.insert(0, inCoreHook())
...
def get_module_filetree(module, dir='.'):
    ...
    files = incore.tools.osutil.listdir(path, True)
...
def load_incore_module(module_name):
    """ Load an inCore module, if not already loaded.
    ...
    initialize_sys_path()
    try:
        __import__('incore.addons.' + module_name)
        ...
        if info['post_load']:
            getattr(sys.modules['incore.addons.' + module_name], info['post_load'])()
...
class TestStream(object):
    def __init__(self, logger_name='incore.tests'):
...
def run_unit_tests(module_name, dbname, position='at_install'):
    ...
    """
    global current_test
    from incore.tests.common import TagsSelector # Avoid import loop
    ...
    for m in mods:
        ...
        if suite.countTestCases():
            ...
            if time.time() - t0 > 5:
                _logger.log(25, "%s tested in %.2fs, %s queries", m.__name__, time.time() - t0, incore.sql_db.sql_counter - t0_sql)
