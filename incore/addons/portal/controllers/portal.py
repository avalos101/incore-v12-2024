...
from incore import fields as incore_fields, tools, _
from incore.exceptions import ValidationError, AccessError, MissingError, UserError
from incore.http import content_disposition, Controller, request, route
from incore.tools import consteq
...
class CustomerPortal(Controller):
    ...
    def _get_archive_groups(self, model, domain=None, fields=None, groupby="create_date", order="create_date desc"):
        ...
        for group in request.env[model]._read_group_raw(domain, fields=fields, groupby=groupby, orderby=order):
            ...
            groups.append({
                'date_begin': incore_fields.Date.to_string(incore_fields.Date.from_string(date_begin)),
                'date_end': incore_fields.Date.to_string(incore_fields.Date.from_string(date_end)),
                'name': label,
