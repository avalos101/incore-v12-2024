...
echo "addons/point_of_sale/tools/posbox/overwrite_after_init/home/pi/incore" >> .git/info/sparse-checkout

git fetch "${localremote}" "${localbranch}" --depth=1
git reset "${localremote}"/"${localbranch}" --hard

git clean -df
cp -a /home/pi/incore/addons/point_of_sale/tools/posbox/overwrite_after_init/home/pi/incore/* /home/pi/incore/
rm -r /home/pi/incore/addons/point_of_sale/tools/posbox/overwrite_after_init
...
(sleep 5 && sudo service incore restart) &
