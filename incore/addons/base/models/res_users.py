...
from incore import api, fields, models, tools, SUPERUSER_ID, _
from incore.exceptions import AccessDenied, AccessError, UserError, ValidationError
from incore.http import request
from incore.osv import expression
from incore.service.db import check_super
from incore.tools import partition, pycompat, collections
...
class Users(models.Model):
    """ User class. A res.users record models an inCore user and is different
        from an employee.
    ...
    def unlink(self):
        if SUPERUSER_ID in self.ids:
            raise UserError(_('You can not remove the admin user as it is used internally for resources created by inCore (updates, module installation, ...)'))
        db = self._cr.dbname
        for id in self.ids:
            self.__uid_cache[db].pop(id, None)
        self._invalidate_session_cache()
    ...
    def change_password(self, old_passwd, new_passwd):
        ...
        :return: True
        :raise: incore.exceptions.AccessDenied when old password is wrong
        :raise: incore.exceptions.UserError when new password is not set or empty
        """
        ...
        if self._on_login_cooldown(failures, previous):
            ...
            if ipaddress.ip_address(source).is_private:
                _logger.warn(
                    ...
                    "and *might* be a proxy. If your inCore is behind a proxy, "
                    "it may be mis-configured. Check that you are running "
                    "inCore in Proxy Mode and that the proxy is properly configured, see "
                    "https://www.incore.co/documentation/12.0/setup/deploy.html#https for details.",
                    source
                )
