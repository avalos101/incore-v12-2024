...
import incore
from incore import api, fields, models, modules, tools, _
from incore.addons.base.models.ir_model import MODULE_UNINSTALL_FLAG
from incore.exceptions import AccessDenied, UserError
from incore.osv import expression
from incore.tools.parse_version import parse_version
from incore.tools.misc import topological_sort
...
class Module(models.Model):
    ...
    author = fields.Char("Editor", readonly=True)
    ...
    license = fields.Selection([
        ...
        ('OEEL-1', 'inCore License v1.0'),
        ('INCORE-1', 'inCore License v1.0'),
        ('OPL-1', 'inCore Proprietary License v1.0'),
        ('Other proprietary', 'Other Proprietary')
    ], string='License', default='INCORE-1', readonly=True)
    ...
    to_buy = fields.Boolean('inCore Enterprise Module', default=False)
    ...
    def install_from_urls(self, urls):
        ...
        INCORE = incore.release.product_name.lower()
        ...
        try:
            ...
            for module_name, url in urls.items():
                ...
                try:
                    _logger.info('Downloading module `%s` from inCore Apps', module_name)
            ...
            for module_name, url in urls.items():
                if module_name == INCORE or not url:
                    continue    # INCORE is special case, handled below, and no URL means local module
            ...
            if urls.get(INCORE):
                ...
                # copy all modules in the SERVER/incore/addons directory to the new "incore" module (except base itself)
                for d in os.listdir(base_path):
                    if d != 'base' and os.path.isdir(os.path.join(base_path, d)):
                        destdir = os.path.join(tmp, INCORE, 'addons', d)    # XXX 'incore' subdirectory ?
                ...
                _logger.info('Copy downloaded module `incore` to `%s`', server_dir)
                shutil.move(os.path.join(tmp, INCORE), server_dir)
                #if bck:
                #    shutil.rmtree(bck)
    ...
    def get_apps_server(self):
        return tools.config.get('apps_server', 'https://apps.incore.co/apps')
