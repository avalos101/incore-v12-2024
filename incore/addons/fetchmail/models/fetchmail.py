...
class FetchmailServer(models.Model):
    ...
    script = fields.Char(readonly=True, default='/mail/static/scripts/incore_mailgate.py')
    ...
    def onchange_server_type(self):
        ...
        self.configuration = """
            ...
            incore_mailgate.py --host=HOSTNAME --port=PORT -u %(uid)d -p PASSWORD -d %(dbname)s
            Example configuration for the postfix mta running locally:
            /etc/postfix/virtual_aliases:
            @youdomain incore_mailgate@localhost
            /etc/aliases:
            incore_mailgate: "|/path/to/incore-mailgate.py --host=localhost -u %(uid)d -p PASSWORD -d %(dbname)s"
        """ % conf
