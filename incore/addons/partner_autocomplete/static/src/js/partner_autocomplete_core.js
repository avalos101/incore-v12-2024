incore.define('partner.autocomplete.core', function (require) {
'use strict';
...
return {
    _dropPreviousinCore: new concurrency.DropPrevious(),
    ...
    autocomplete: function (value) {
        ...
        var incorePromise = this._getinCoreSuggestions(value, isVAT).then(function (suggestions){
            incoreSuggestions = suggestions;
        });
        ...
        var concatResults = function () {
            // Add Clearbit result with inCore result (with unique domain)
            if (clearbitSuggestions && clearbitSuggestions.length) {
                var websites = incoreSuggestions.map(function (suggestion) {
                ...
                clearbitSuggestions.forEach(function (suggestion) {
                    if (websites.indexOf(suggestion.domain) < 0) {
                        ...
                        incoreSuggestions.push(suggestion);
                    }
                });
            }

            incoreSuggestions = _.filter(incoreSuggestions, function (suggestion) {
                return !suggestion.ignored;
            });
            _.each(incoreSuggestions, function(suggestion){
              delete suggestion.ignored;
            });
            return def.resolve(incoreSuggestions);
        };

        this._whenAll([incorePromise, clearbitPromise]).then(concatResults, concatResults);
    ...
    /**
     * Use inCore Autocomplete API to return suggestions
     *
     * @param {string} value
     * @param {boolean} isVAT
     * @returns {Deferred}
     * @private
     */
    _getinCoreSuggestions: function (value, isVAT) {
        ...
        return this._dropPreviousinCore.add(def);
    },
    /**
