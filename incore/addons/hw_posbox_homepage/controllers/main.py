...
import io
import os
from incore.tools import misc
import urllib3
from pathlib import Path

from uuid import getnode as get_mac
from incore.addons.hw_proxy.controllers import main as hw_proxy
from incore.addons.web.controllers import main as web
from incore.modules.module import get_resource_path
from incore.addons.hw_drivers.controllers.driver import iot_devices, get_ip, get_incore_server_url
...
if hasattr(sys, 'frozen'):
    ...
    loader = jinja2.PackageLoader('incore.addons.hw_posbox_homepage', "views")
...
class IoTboxHomepage(web.Home):
    ...
    def get_hw_screen_message(self):
        ...
        <p>
            ...
            You can find the latest images on the <a href="http://nightly.incore.co/master/posbox/">inCore Nightly builds</a> website.
            Make sure to download at least the version 16.<br/>
            inCore version 11, or above, is required to use the customer display feature.
        </p>
        """
    ...
    def get_homepage_data(self):
        ...
        return {
            ...
            'server_status': get_incore_server_url() or 'Not Configured',
    ...
    def index(self):
        ...
        remote_server = Path.home() / 'incore-remote-server.conf'
    ...
    def list_drivers(self):
        ...
        return driver_list_template.render({
            'title': "inCore's IoT Box - Drivers list",
    ...
    def load_drivers(self):
        ...
        subprocess.check_call("sudo service incore restart", shell=True)
        ...
        return "<meta http-equiv='refresh' content='20; url=http://" + get_ip() + ":8080/list_drivers'>"
    ...
    def connect_to_wifi(self, essid, password, persistent=False):
        ...
        if server:
            res_payload['server'] = {
                'url': server,
                'message': 'Redirect to inCore Server'
            }
    ...
    def clear_wifi_configuration(self):
        ...
        return "<meta http-equiv='refresh' content='0; url=http://" + get_ip() + ":8080'>"
    ...
    def clear_drivers_list(self):
        ...
        return "<meta http-equiv='refresh' content='0; url=http://" + get_ip() + ":8080/list_drivers'>"
    ...
    def server(self):
        return server_config_template.render({
            'title': 'IoT -> inCore server configuration',
            'breadcrumb': 'Configure inCore Server',
            'hostname': subprocess.check_output('hostname').decode('utf-8').strip('\n'),
            'server_status': get_incore_server_url() or 'Not configured yet',
    ...
    def remote_connect(self):
        ...
        2 - copy past this auth_token on the IoT Box : http://IoT_Box:8080/remote_connect
