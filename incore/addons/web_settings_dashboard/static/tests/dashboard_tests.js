incore.define('web_settings_dashboard.settings_dashboard_tests', function (require) {
"use strict";
...
QUnit.module('settings_dashboard', function () {
    QUnit.test('Dashboard: Invite new user', function (assert) {
        ...
        // add email to invite
        dashboard.$('.o_user_emails').val('lagan@incore.co').trigger($.Event('keydown', {
            which: $.ui.keyCode.ENTER,
        }));
        assert.strictEqual(dashboard.$('.o_badge_text').text().trim(), 'lagan@incore.co',
        ...
        assert.strictEqual(dashboard.$('.o_web_settings_dashboard_user').text().trim(), 'lagan@incore.co',
    ...
    QUnit.test('Dashboard: Invite new user (warnings)', function (assert) {
        ...
        dashboard.$('.o_user_emails').val('xyz@incore.co').trigger($.Event('keydown', {
            which: $.ui.keyCode.ENTER,
        }));
        assert.strictEqual(dashboard.$('.o_badge_text').length, 0,
            'should not have generated any badge');
        assert.strictEqual(dashboard.$('.o_user_emails').val(), 'xyz@incore.co',
            'input should not have been cleared');
    ...
    QUnit.test('Dashboard: Invite a list of users', function (assert) {
        ...
        var emails = ['a@incore.co', 'b@incore.co', 'c@incore.co', 'd@incore.co'];
    ...
    QUnit.test('Dashboard: Invite a list of users (with warnings)', function (assert) {
                ...
                if (route === '/web_settings_dashboard/data') {
                    return $.when({
                        ...
                        users_info: {
                            active_users: 4,
                            pending_counts: 1,
                            pending_users: [[4, 'a@incore.co'], [5, 'd@incore.co']],
                        },
                    });
                }
                return this._super.apply(this, arguments);
        ...
        // simulate a copy paste of multiple email addresses
        var emails = ['a@incore.co', 'b@incore.co', 'x@y', 'd@incore.co'];
        dashboard.$('.o_user_emails').val(emails.join(' ')).trigger($.Event('keydown', {
            which: $.ui.keyCode.ENTER,
        }));
