incore.define('web_settings_dashboard', function (require) {
"use strict";
...
var DashboardApps = Widget.extend({
    ...
    start: function() {
        ...
        if (incore.db_info && _.last(incore.db_info.server_version_info) !== 'e') {
    ...
    confirm_upgrade: function() {
        framework.redirect("https://www.incore.co/incore-enterprise/upgrade?num_users=" + (this.data.enterprise_users || 1));
    },
});
...
var DashboardShare = Widget.extend({
    ...
    init: function (parent, data) {
        this.data = data;
        this.parent = parent;
        this.share_url = 'https://www.incore.co';
        this.share_text = encodeURIComponent("I am using #inCore - Awesome open source business apps.");
        return this._super.apply(this, arguments);
    },
    ...
    share_linkedin: function (ev) {
        ev.preventDefault();
        var popup_url = _.str.sprintf('http://www.linkedin.com/shareArticle?mini=true&url=%s&title=I am using incore&summary=%s&source=www.incore.co', encodeURIComponent(this.share_url), this.share_text);
        this.sharer(popup_url);
    },
