...
from incore.addons.website.tools import MockRequest


@incore.tests.tagged('post_install', '-at_install')
class TestUi(incore.tests.HttpCase):
    def test_01_admin_shop_tour(self):
        self.phantom_js("/", "incore.__DEBUG__.services['web_tour.tour'].run('shop')", "incore.__DEBUG__.services['web_tour.tour'].tours.shop.ready", login="admin")

    def test_02_admin_checkout(self):
        self.phantom_js("/", "incore.__DEBUG__.services['web_tour.tour'].run('shop_buy_product')", "incore.__DEBUG__.services['web_tour.tour'].tours.shop_buy_product.ready", login="admin")

    def test_03_demo_checkout(self):
        self.phantom_js("/", "incore.__DEBUG__.services['web_tour.tour'].run('shop_buy_product')", "incore.__DEBUG__.services['web_tour.tour'].tours.shop_buy_product.ready", login="demo")


@incore.tests.tagged('post_install', '-at_install')
class TestWebsiteSaleCheckoutAddress(incore.tests.TransactionCase):
