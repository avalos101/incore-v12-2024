incore.define('mail.Manager', function (require) {
"use strict";
...
var MailManager =  AbstractService.extend({
    ...
    _INCOREBOT_ID: "INCOREBOT", // default authorID for transient messages
    ...
    /**
     * Get the inCoreBot ID, which is the default authorID for transient messages
     *
     * @returns {string}
     */
    getinCorebotID: function () {
        return this._INCOREBOT_ID;
    },
    /**
            ...
            if (thread) {
                if (
                    ...
                    if (thread.isTwoUserThread() && options.showNotification) {
                        ...
                        if (!self.call('bus_service', 'isinCoreFocused') || !query.isVisible) {
    ...
    _initializeInternalState: function () {
        ...
        // # of message received when incore is out of focus
    ...
    _notifyIncomingMessage: function (message) {
        if (this.call('bus_service', 'isinCoreFocused')) {
            // no need to notify
            return;
        }
