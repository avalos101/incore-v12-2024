...
from incore import _, api, fields, models
from incore.exceptions import ValidationError
from incore.tools import remove_accents
...
class Alias(models.Model):
    """A Mail Alias is a mapping of an email address with a given inCore Document
       model. It is used by inCore's mail gateway when processing incoming emails
       ...
       created, it becomes immediately usable and inCore will accept email for it.
     """
    _name = 'mail.alias'
    ...
    alias_name = fields.Char('Alias Name', help="The name of the email alias, e.g. 'jobs' if you want to catch emails for <jobs@example.incore.co>")
    alias_model_id = fields.Many2one('ir.model', 'Aliased Model', required=True, ondelete="cascade",
                                     help="The model (inCore Document Kind) to which this alias "
    ...
    def name_get(self):
        """Return the mail alias display alias_name, including the implicit
           ...
           e.g. `jobs@mail.incore.co` or `jobs` or 'New Alias'
        """
        res = []
        for record in self:
