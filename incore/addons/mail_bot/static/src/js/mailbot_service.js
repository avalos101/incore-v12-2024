incore.define('mail_bot.MailBotService', function (require) {
"use strict";
...
var MailBotService =  AbstractService.extend({
    ...
    start: function () {
        ...
        if ('incorebot_initialized' in session && ! session.incorebot_initialized) {
            this._showinCorebotTimeout();
        }
    },
    ...
    /**
     * Get the previews related to the inCoreBot (conversation not included).
     * For instance, when there is no conversation with inCoreBot and inCoreBot has
    ...
    getPreviews: function (filter) {
        ...
        if (this.hasRequest() && (filter === 'mailbox_inbox' || !filter)) {
            previews.push({
                title: _t("inCoreBot has a request"),
                imageSRC: "/mail/static/src/img/incorebot.png",
                status: 'bot',
    ...
    /**
     * Tell whether inCoreBot has a request or not.
     *
     * @returns {boolean}
     */
    hasRequest: function () {
     ...
     * @private
     */
    _showinCorebotTimeout: function () {
        var self = this;
        setTimeout(function () {
            session.incorebot_initialized = true;
            self._rpc({
                model: 'mail.channel',
                method: 'init_incorebot',
            });
        }, 2*60*1000);
    },
});
