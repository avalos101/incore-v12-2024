...
from incore.addons.payment.tests.common import PaymentAcquirerCommon
from incore.addons.payment_authorize.controllers.main import AuthorizeController
...
@incore.tests.tagged('post_install', '-at_install', '-standard', 'external')
class AuthorizeForm(AuthorizeCommon):
    ...
    @mute_logger('incore.addons.payment_authorize.models.payment', 'ValidationError')
...
@incore.tests.tagged('post_install', '-at_install', '-standard')
class AuthorizeForm(AuthorizeCommon):
    def test_30_authorize_s2s(self):
        ...
        # add credential
        # FIXME: put this test in master-nightly on incore/incore + create sandbox account
        authorize.write({
