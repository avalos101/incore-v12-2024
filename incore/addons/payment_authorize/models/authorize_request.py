...
from incore.addons.payment.models.payment_acquirer import _partner_split_name
from incore.exceptions import ValidationError, UserError
from incore import _
...
class AuthorizeAPI():
    ...
    def create_customer_profile(self, partner, cardnumber, expiration_date, card_code):
        ...
        a corresponding payment profile to it. Note that a single partner in the inCore
        ...
        # merchantCustomerId is INCORE-{partner.id}-{random hex string} truncated to maximum 20 characters
        etree.SubElement(profile, "merchantCustomerId").text = ('INCORE-%s-%s' % (partner.id, uuid4().hex[:8]))[:20]
    ...
    def create_customer_profile_from_tx(self, partner, transaction_id):
        ...
        # merchantCustomerId is INCORE-{partner.id}-{random hex string} truncated to maximum 20 characters
        etree.SubElement(customer, "merchantCustomerId").text = ('INCORE-%s-%s' % (partner.id, uuid4().hex[:8]))[:20]
