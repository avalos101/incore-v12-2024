...
example in inCore is dashboards), queries should be restricted to a
...
inCore Web's :js:class:`~incore.web.Widget` has an attribute
providing its DOM root :js:attr:`Widget.$el <incore.web.Widget.$el>`,
...
* All interactive components (components displaying information to the
  ...
  :class:`~incore.web.Widget` and correctly implement and use its API
  and lifecycle.
...
From v11, we introduce a new coding standard for inCore Javascript code.  Here it
is:

* add "use strict"; on top of every incore JS module
