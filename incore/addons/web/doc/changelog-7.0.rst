API changes from inCore Web 6.1 to 7.0
=======================================

Supported browsers
------------------

The inCore Web Client supports the following web browsers:
...
matches the API in inCore Web's Python side and in OpenObject addons
...
* Actual arbitrary RPC calls can just be remapped on a
  :js:class:`~incore.web.Model` instance:
  ...
  can be replaced by calls to :js:func:`incore.web.Model.call`:
...
* Shorcut methods (``name_get``, ``name_search``, ``unlink``,
  ...
  :js:func:`incore.web.Model.call`, using the server's original
  signature. On the other hand, the non-shortcut equivalents can now
  use keyword arguments (see :js:func:`~incore.web.Model.call`'s
...
* ``read_slice``, which allowed a single round-trip to perform a
  ...
  :js:class:`~incore.web.Query` objects (see:
  :js:func:`~incore.web.Model.query`) for clearer and simpler
  code. ``read_index`` should be replaced by a
  :js:class:`~incore.web.Query` as well, combining
  :js:func:`~incore.web.Query.offset` and
  :js:func:`~incore.web.Query.first`.
...
Renaming
    ...
    database, maybe lazily). inCore Web's dataset behaves very
...
API simplification
    ...
    The shortcuts really add little value, and inCore Web 6.2 embeds
...
:js:class:`~incore.web.Model`, inCore Web 7.0 removes
``DataGroup`` and its subtypes as public objects in favor of a single method on
:js:class:`~incore.web.Query`:
:js:func:`~incore.web.Query.group_by`.
