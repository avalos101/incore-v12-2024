...
A very basic inCore module structure will be our starting point:
...
There is no such thing as a "web module" declaration. An inCore
...
.. note::
    ...
    restart the inCore server itself for it to be taken in account.
...
provides: an instance of the inCore Web client, with the contents of
...
its :js:func:`~incore.web.Widget.start` to add some content to its
DOM:
...
will require restarting the inCore Server to see the changes, as
usual):
...
.. note::
    ...
    Prefixing CSS rules with both ``.incore`` (to ensure the rule
    will apply only within the confines of the inCore Web client) and
...
be done in :js:func:`~incore.web.Widget.start` but that gets unwieldy
...
inCore Web's template language is :doc:`qweb`. Although any
...
integration to inCore Web widgets.
...
To prevent this irremediable loss, we can use inCore's support for
...
:js:func:`~incore.base.Widget.start()` is to perform *asynchronous*
...
recorded previously using the :js:class:`~incore.web.Query` interface
...
4. And finally — after restarting inCore — navigating to the test
...
side-effect of this is to :js:func:`~incore.web.Widget.start` it,
...
:js:func:`set a widget's DOM <incore.web.Widget.setElement>`, let's
