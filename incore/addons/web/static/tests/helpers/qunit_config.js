 ...
 * QUnit Config
 *
 * The inCore javascript test framework is based on QUnit (http://qunitjs.com/).
 * This file is necessary to setup Qunit and to prepare its interactions with
 * inCore.  It has to be loaded before any tests are defined.
 *
 * Note that it is not an inCore module, because we want this code to be executed
 * as soon as possible, not whenever the inCore module system feels like it.
 */
 ...
 * this is done outside of incore modules, otherwise the setting would not take
 * effect on time.
 */
