...
(function() {
    ...
    var incore = window.incore = window.incore || {};
    _.extend(incore, {
            ...
            if (jobs.length) {
                ...
                for (var k=0; k<jobs.length; k++) {
                    debug_jobs[jobs[k].name] = job = {
                        ...
                        dependents: incore.__DEBUG__.get_dependents(jobs[k].name),
                        name: jobs[k].name
                    };
                    if (jobs[k].error) {
                    ...
                    var deps = incore.__DEBUG__.get_dependencies( job.name );
                ...
                missing = incore.__DEBUG__.get_missing_jobs();
                failed = incore.__DEBUG__.get_failed_jobs();
                var unloaded = _.filter(debug_jobs, function (job) { return job.missing; });

                if (incore.debug || !_.isEmpty(failed) || !_.isEmpty(unloaded)) {
                    ...
                    if (incore.debug && !_.isEmpty(debug_jobs)) {
                        log('Debug:                   ', debug_jobs);
                    }
                }
            }
            incore.__DEBUG__.js_modules = {
                missing: missing,
                failed: _.pluck(failed, 'name'),
            };
            incore.__DEBUG__.didLogInfo.resolve();
        },
                ...
                if (!job.error) {
                    $.when(job_exec).then(
                        function (data) {
                            ...
                            def.resolve();
                            incore.process_jobs(jobs, services);
                        }, function (e) {
                            job.rejected = e || true;
                            jobs.push(job);
                            def.resolve();
