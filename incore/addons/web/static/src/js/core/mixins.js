incore.define('web.mixins', function (require) {
"use strict";
...
function inCoreEvent(target, name, data) {
    this.target = target;
    this.name = name;
...
inCoreEvent.prototype.stopPropagation = function () {
    this.stopped = true;
};

inCoreEvent.prototype.is_stopped = function () {
    return this.stopped;
};
...
var EventDispatcherMixin = _.extend({}, ParentedMixin, {
     ...
     * inCore Web where developers may want to replace existing callbacks with
     * theirs.
     *
    ...
    trigger_up: function (name, info) {
        var event = new inCoreEvent(this, name, info);
