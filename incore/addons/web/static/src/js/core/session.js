incore.define('web.Session', function (require) {
"use strict";
...
var Session = core.Class.extend(mixins.EventDispatcherMixin, {
    ...
    init: function (parent, origin, options) {
        ...
        options = options || {};
        this.module_list = (options.modules && options.modules.slice()) || (window.incore._modules && window.incore._modules.slice()) || [];
        this.server = null;
    ...
    on_modules_loaded: function () {
        ...
        for(var j=0; j<this.module_list.length; j++) {
            ...
            incore[mod] = {};
            // init module mod
            var fct = incore._incore[mod];
            if(typeof(fct) === "function") {
                incore._incore[mod] = {};
                for (var k in fct) {
                    incore._incore[mod][k] = fct[k];
                }
                fct(incore, incore._incore[mod]);
            }
    ...
    session_reload: function () {
        var result = _.extend({}, window.incore.session_info);
        delete result.session_id;
        ...
        var promise = deferred.then(function () {
            ...
            if (self.origin_server) {
                ...
                if (self.override_session) {
                    options.headers["X-Incore-Session-Id"] = self.session_id || '';
                }
