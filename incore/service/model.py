...
import incore
from incore.exceptions import UserError, ValidationError, QWebException
from incore.models import check_method_name
from incore.tools.translate import translate
...
def dispatch(method, params):
    ...
    registry = incore.registry(db).check_signaling()
    fn = globals()[method]
...
def check(f):
    ...
    def wrapper(___dbname, *args, **kwargs):
        ...
        def tr(src, ttype):
            ...
            try:
                cr = incore.sql_db.db_connect(dbname).cursor()
        ...
        while True:
            try:
                if incore.registry(dbname)._init and not incore.tools.config['test_enable']:
                    raise incore.exceptions.Warning('Currently, this database is not fully loaded and can not be used.')
...
def execute_cr(cr, uid, obj, method, *args, **kw):
    recs = incore.api.Environment(cr, uid, {}).get(obj)
    if recs is None:
        raise UserError(_("Object %s doesn't exist") % obj)
    return incore.api.call_kw(recs, method, args, kw)
...
def execute(db, uid, obj, method, *args, **kw):
    ...
    with incore.registry(db).cursor() as cr:
