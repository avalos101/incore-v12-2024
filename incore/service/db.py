...
import incore.sql_db
import incore.tools
from incore.sql_db import db_connect
from incore.release import version_info
...
def check_db_management_enabled(method):
    def if_db_mgt_enabled(method, self, *args, **kwargs):
        if not incore.tools.config['list_db']:
...
def check_super(passwd):
    if passwd and incore.tools.config.verify_admin_password(passwd):
        return True
    raise incore.exceptions.AccessDenied()

# This should be moved to incore.modules.db, along side initialize().
def _initialize_db(id, db_name, demo, lang, user_password, login='admin', country_code=None, phone=None):
    try:
        db = incore.sql_db.db_connect(db_name)
        with closing(db.cursor()) as cr:
            # TODO this should be removed as it is done by Registry.new().
            incore.modules.db.initialize(cr)
            incore.tools.config['load_language'] = lang
            cr.commit()

        registry = incore.modules.registry.Registry.new(db_name, demo, None, update_module=True)

        with closing(db.cursor()) as cr:
            env = incore.api.Environment(cr, SUPERUSER_ID, {})
            ...
            if login:
                values['login'] = login
                emails = incore.tools.email_split(login)
                if emails:
...
def _create_empty_database(name):
    db = incore.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        chosen_template = incore.tools.config['db_template']
...
def exp_duplicate_database(db_original_name, db_name):
    ...
    incore.sql_db.close_db(db_original_name)
    db = incore.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        cr.autocommit(True)     # avoid transaction block
    ...
    registry = incore.modules.registry.Registry.new(db_name)
    with registry.cursor() as cr:
        # if it's a copy of a database, force generation of a new dbuuid
        env = incore.api.Environment(cr, SUPERUSER_ID, {})
        env['ir.config_parameter'].init(force=True)

    from_fs = incore.tools.config.filestore(db_original_name)
    to_fs = incore.tools.config.filestore(db_name)
...
def exp_drop(db_name):
    if db_name not in list_dbs(True):
        return False
    incore.modules.registry.Registry.delete(db_name)
    incore.sql_db.close_db(db_name)
    ...
    fs = incore.tools.config.filestore(db_name)
...
def dump_db_manifest(cr):
    ...
    manifest = {
        'incore_dump': '1',
        'db_name': cr.dbname,
        'version': incore.release.version,
        'version_info': incore.release.version_info,
        'major_version': incore.release.major_version,
...
def dump_db(db_name, stream, backup_format='zip'):
    ...
    if backup_format == 'zip':
        with incore.tools.osutil.tempdir() as dump_dir:
            filestore = incore.tools.config.filestore(db_name)
            ...
            incore.tools.exec_pg_command(*cmd)
            if stream:
                incore.tools.osutil.zip_dir(dump_dir, stream, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
            else:
                t=tempfile.TemporaryFile()
                incore.tools.osutil.zip_dir(dump_dir, t, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
                t.seek(0)
                return t
    else:
        cmd.insert(-1, '--format=c')
        stdin, stdout = incore.tools.exec_pg_command_pipe(*cmd)
        if stream:
...
def restore_db(db, dump_file, copy=False):
    ...
    with incore.tools.osutil.tempdir() as dump_dir:
        ...
        if incore.tools.exec_pg_command(pg_cmd, *pg_args):
            raise Exception("Couldn't restore database")

        registry = incore.modules.registry.Registry.new(db)
        with registry.cursor() as cr:
            env = incore.api.Environment(cr, SUPERUSER_ID, {})
            if copy:
                # if it's a copy of a database, force generation of a new dbuuid
            ...
            if incore.tools.config['unaccent']:
                try:
                    with cr.savepoint():
...
def exp_rename(old_name, new_name):
    incore.modules.registry.Registry.delete(old_name)
    incore.sql_db.close_db(old_name)
    ...
    old_fs = incore.tools.config.filestore(old_name)
    new_fs = incore.tools.config.filestore(new_name)
...
def exp_change_admin_password(new_password):
    incore.tools.config.set_admin_password(new_password)
    incore.tools.config.save()
    return True
...
def exp_migrate_databases(databases):
    for db in databases:
        ...
        incore.tools.config['update']['base'] = True
        incore.modules.registry.Registry.new(db, force_demo=False, update_module=True)
    return True
...
@incore.tools.mute_logger('incore.sql_db')
...
def list_dbs(force=False):
    if not incore.tools.config['list_db'] and not force:
        raise incore.exceptions.AccessDenied()

    if not incore.tools.config['dbfilter'] and incore.tools.config['db_name']:
        # In case --db-filter is not provided and --database is passed, inCore will not
        ...
        res = sorted(db.strip() for db in incore.tools.config['db_name'].split(','))
        return res
    ...
    with closing(db.cursor()) as cr:
        try:
            ...
            res = [incore.tools.ustr(name) for (name,) in cr.fetchall()]
        except Exception:
...
def list_db_incompatible(databases):
    """"Check a list of databases if they are compatible with this version of inCore
    ...
    for database_name in databases:
        with closing(db_connect(database_name).cursor()) as cr:
            if incore.tools.table_exists(cr, 'ir_module_module'):
    ...
    for database_name in incompatible_databases:
        # release connection
        incore.sql_db.close_db(database_name)
...
def exp_list_lang():
    return incore.tools.scan_languages()

def exp_list_countries():
    list_countries = []
    root = ET.parse(os.path.join(incore.tools.config['root_path'], 'addons/base/data/res_country_data.xml')).getroot()
