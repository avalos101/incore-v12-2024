...
import incore
from incore.modules import get_modules
from incore.modules.module import run_unit_tests, get_test_modules
from incore.modules.registry import Registry
from incore.release import nt_service_name
from incore.tools import config
from incore.tools import stripped_sys_argv, dumpstacks, log_ormcache_stats
...
class RequestHandler(werkzeug.serving.WSGIRequestHandler):
    def setup(self):
        ...
        me.name = 'incore.service.http.request.%s' % (me.ident,)
    ...
    def handle_file(self, path):
        if path.endswith('.py') and not os.path.basename(path).startswith('.~'):
            try:
                ...
                if not getattr(incore, 'phoenix', False):
...
class FSWatcherWatchdog(FSWatcherBase):
    def __init__(self):
        ...
        for path in incore.modules.module.ad_paths:
...
class FSWatcherInotify(FSWatcherBase):
    ...
    def start(self):
        self.started = True
        self.thread = threading.Thread(target=self.run, name="incore.service.autoreload.watcher")
...
class ThreadedServer(CommonServer):
    ...
    def cron_thread(self, number):
        from incore.addons.base.models.ir_cron import ir_cron
        while True:
            time.sleep(SLEEP_INTERVAL + number)     # Steve Reich timing style
            registries = incore.modules.registry.Registry.registries
    ...
    def cron_spawn(self):
        ...
        for i in range(incore.tools.config['max_cron_threads']):
            def target():
                self.cron_thread(i)
            t = threading.Thread(target=target, name="incore.service.cron.cron%d" % i)
            t.setDaemon(True)
            t.type = 'cron'
            t.start()
    ...
    def http_spawn(self):
        t = threading.Thread(target=self.http_thread, name="incore.service.httpd")
        t.setDaemon(True)
        t.start()
    ...
    def stop(self):
        ...
        """
        if getattr(incore, 'phoenix', None):
        ...
        _logger.debug('--')
        incore.modules.registry.Registry.delete_all()
        logging.shutdown()
...
class Worker(object):
    ...
    def setproctitle(self, title=""):
        setproctitle('incore: %s %s %s' % (self.__class__.__name__, self.pid, title))
    ...
    def run(self):
        try:
            ...
            _logger.info("Worker (%s) exiting. request_count: %s, registry count: %s.",
                         ...
                         len(incore.modules.registry.Registry.registries))
            self.stop()
        except Exception:
...
class WorkerCron(Worker):
    ...
    def _db_list(self):
        if config['db_name']:
            ...
            db_names = incore.service.db.list_dbs(True)
        return db_names

    def process_work(self):
        rpc_request = logging.getLogger('incore.netsvc.rpc.request')
        ...
        if len(db_names):
            ...
            from incore.addons import base
...
def load_server_wide_modules():
    server_wide_modules = {'base', 'web'} | set(incore.conf.server_wide_modules)
    for m in server_wide_modules:
        try:
            incore.modules.module.load_incore_module(m)
        except Exception:
            msg = ''
            if m == 'web':
                msg = """
The `web` module is provided by the addons found in the `incore-web` project.
Maybe you forgot to add those addons in your addons_path configuration."""
            _logger.exception('Failed to load server-wide module `%s`.%s', m, msg)

def _reexec(updated_modules=None):
    """reexecute incore-server process with (nearly) the same arguments"""
    if incore.tools.osutil.is_running_as_nt_service():
        ...
        for mod in [m for m in get_modules() if '/%s/' % m in test_file]:
            for mod_mod in get_test_modules(mod):
                ...
                if test_path == mod_path:
                    ...
                    stream = incore.modules.module.TestStream()
    ...
    for dbname in dbnames:
        try:
            ...
            if config['test_enable']:
                t0 = time.time()
                t0_sql = incore.sql_db.sql_counter
                ...
                _logger.info("All post-tested in %.2fs, %s queries",
                             time.time() - t0, incore.sql_db.sql_counter - t0_sql)
...
def start(preload=None, stop=False):
    """ Start the incore http server and cron processor.
    """
    global server

    load_server_wide_modules()
    incore.service.wsgi_server._patch_xmlrpc_marshaller()

    if incore.evented:
        server = GeventServer(incore.service.wsgi_server.application)
    elif config['workers']:
        if config['test_enable'] or config['test_file']:
            _logger.warning("Unit testing in workers mode could fail; use --workers 0.")

        server = PreforkServer(incore.service.wsgi_server.application)
        ...
        server = ThreadedServer(incore.service.wsgi_server.application)

    watcher = None
    if 'reload' in config['dev_mode'] and not incore.evented:
        if inotify:
    ...
    if getattr(incore, 'phoenix', False):
        _reexec()
