...
Official compatibility: inCore 11 will be the first LTS release to introduce
...
used by the official inCore distribution have been ported and are considered
...
.. important::
    ...
    a guide to write 2/3-compatible inCore code. It does not go through all the
    changes in Python but rather through issues which have been found in the
    standard inCore distribution in order to show how to evolve such code such
...
A cross compatible inCore would only support Python 2.7 and Python 3.5 and
...
.. warning::
    ...
    generator delegation, pathlib, ...), you must *not* use them in inCore
    until Python 2 support is dropped

.. note::

    In the *very rare* cases where you *need* to differentiate between
    Python 2 and Python 3, use the :data:`incore.tools.pycompat.PY2` flag.
...
* fixing dependencies
* using a ``collections.OrderedDict`` or ``incore.tools.misc.OrderedSet`` instead
...
* ``urllib``, ``urllib2`` and ``urlparse`` were redistributed across
  ...
  Since `requests`_ and `werkzeug`_ are already hard dependencies of inCore,
...
* ``cgi.escape`` (HTML escaping) is deprecated in Python 3, prefer inCore's own
  :func:`incore.tools.misc.html_encode`.
...
actually happened in inCore with :mod:`incore.tools.mimetypes`.
...
.. important::
    ...
    * ``(int, long)`` for type-checking purposes must be replaced by
      :py:data:`incore.tools.pycompat.integer_types`
...
* type-testing is the last and bigger issue as in Python 2 ``long`` is not a
  ...
  For that case, inCore 11 now provides a compatibility module with an
  :py:data:`~incore.tools.pycompat.integer_types` definition which can be used
  for type-testing.
...
For easier type-testing, :mod:`incore.tools.pycompat` provides the following
constants:

* :data:`~incore.tools.pycompat.string_types` is an alias/type tuple for testing
  string types, essentially a replacement of testing for ``basestring`` or
  ``(str, unicode)``.
* :data:`~incore.tools.pycompat.text_type` is the proper *text* type for the
...
However with respect to inCore it turns out most or all uses of ``csv`` fit
inside a model of *byte stream to and from text values*.

The latter is thus a model implemented by cross-version wrappers
:func:`incore.tools.pycompat.csv_reader` and
:func:`incore.tools.pycompat.csv_writer`: they take a *UTF-8 byte stream* and
