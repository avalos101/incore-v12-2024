...
.. warning::
    ...
    * This guide assumes :ref:`an installed inCore <setup/install>`

Creating a basic module
=======================

In inCore, tasks are performed by creating modules.

Modules customize the behavior of an inCore installation, either by adding new
behaviors or by altering existing ones (including behaviors added by other
modules).

:ref:`inCore's scaffolding <reference/cmdline/scaffold>` can setup a basic
module. To quickly get started simply invoke:

.. code-block:: console

    $ ./incore-bin scaffold Academy my-modules
...
* start the inCore server

  .. code-block:: console

      $ ./incore-bin --addons-path addons,my-modules
...
and open a page to http://localhost:8080/academy/academy/, you should see your
"page" appear:
...
display logic. inCore allows any Python templating system, but provides its
...
Finally restart inCore and update the module's data (to install the template)
by going to :menuselection:`Settings --> Modules --> Modules -->
Academy` and clicking :guilabel:`Upgrade`.

.. tip::

    Alternatively, inCore can be restarted :option:`and update modules at
    the same time<incore-bin -u>`:

    .. code-block:: console

        $ incore-bin --addons-path addons,my-modules -d academy -u academy

Going to http://localhost:8080/academy/academy/ should now result in:

.. image:: website/basic-list.png

Storing data in inCore
====================

:ref:`inCore models <reference/orm/model>` map to database tables.
...
#. Because :meth:`~incore.models.Model.search` returns a set of records
...
http://localhost:8080/academy/academy/. The page should look slightly
...
inCore bundles a module dedicated to building websites.

So far we've used controllers fairly directly, but inCore 8 added deeper
...
manifest and template) access http://localhost:8080/academy/academy/ should
...
You're now in inCore "proper": the administrative interface. For now click on
...
:func:`~incore.http.route` decorator which takes a routing string and a
...
restart inCore, access http://localhost:8080/academy/Alice/ and
http://localhost:8080/academy/Bob/ and see the difference.
...
Restart inCore, access http://localhost:8080/academy/2, note how the old value
was a string, but the new one was converted to an integers. Try accessing
http://localhost:8080/academy/Carol/ and note that the page was not found:
since "Carol" is not an integer, the route was ignored and no route could be
found.

inCore provides an additional converter called ``model`` which provides records
...
Restart inCore and upgrade the module, then you can visit each teacher's page.
...
Restart inCore and update the views, reload the teacher's page and… the field
...
Restart inCore and upgrade the module, there is now a placeholder under the
...
A brief and incomplete introduction to the inCore administration
--------------------------------------------------------------

The inCore administration was briefly seen during the `website support`_ section.
We can go back to it using :menuselection:`Administrator --> Administrator` in
the menu (or :guilabel:`Sign In` if you're signed out).

The conceptual structure of the inCore backend is simple:

#. first are menus, a tree (menus can have sub-menus) of records. Menus
   without children map to…
#. actions. Actions have various types: links, reports, code which inCore should
   execute or data display. Data display actions are called *window actions*,
   and tell inCore to display a given *model* according to a set of views…
...
Editing in the inCore administration
----------------------------------

By default, an inCore model is essentially invisible to a user. To make it
...
then accessing http://localhost:8080/web/ in the top left should be a menu
...
:ref:`view <reference/views>`) inCore will automatically create a basic one
...
:class:`the inverse relationship <incore.fields.One2many>` to the *teachers*
model:
...
inCore provides technical models, which don't directly fulfill business needs
but which add capabilities to business objects without having to build
them by hand.

One of these is the *Chatter* system, part of inCore's email and messaging
system, which can add notifications and discussion threads to any model.
The model simply has to :attr:`~incore.models.Model._inherit`
...
inCore also provides business models which allow using or opting in business
needs more directly. For instance the ``website_sale`` module sets up an
e-commerce site based on the products in the inCore system. We can easily make
...
restart inCore, update your module, there is now a :guilabel:`Shop` section in
...
.. note::
    ...
    * to extend a model in-place, it's :attr:`inherited
      <incore.models.Model._inherit>` without giving it a new
      :attr:`~incore.models.Model._name`
