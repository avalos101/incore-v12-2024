===================
Profiling inCore code
===================

.. warning::

    This tutorial requires :ref:`having installed inCore <setup/install>`
    and :doc:`writing inCore code <backend>`

Graph a method
==============

inCore embeds a profiler of code. This embeded profiler output can be used to
...
.. code:: python
    ...
    from incore.tools.profiler import profile
    [...]
...
.. code:: bash
    ...
    2018-03-28 06:18:23,196 22878 INFO incore incore.tools.profiler:
    calls     queries   ms
    project.task ------------------------ /home/incore/src/incore/addons/project/models/project.py, 638
...
Sending the SIGQUIT signal to an inCore process (only available on POSIX) makes
this process output the current stack trace to log, with info level. When an
incore process seems stucked, sending this signal to the process permit to know
what the process is doing, and letting the process continue his job.

Tracing code execution
======================

Instead of sending the SIGQUIT signal to an inCore process often enough, to check
...
where <PID> is the process ID of the incore process you want to graph. This will
