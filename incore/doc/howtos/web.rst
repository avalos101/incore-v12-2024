...
This guide is about creating modules for inCore's web client.

To create websites with inCore, see :doc:`website`; to add business capabilities
or extend existing business systems of inCore, see :doc:`backend`.
...
.. warning::
    ...
    * `Underscore.js`_
    ...
    It also requires :ref:`an installed inCore <setup/install>`, and Git_.


A Simple Module
===============

Let's start with a simple inCore module holding basic web component
...
.. code-block:: console
    ...
    $ git clone http://github.com/incore/petstore

This will create a ``petstore`` folder wherever you executed the command.
You then need to add that folder to inCore's
:option:`addons path <incore-bin --addons-path>`, create a new database and
...
Files used in the "web" side of an inCore module must be placed in a ``static``
...
``oepetstore/static/js/petstore.js``
    ...
    currently look like::
        ...
        incore.oepetstore = function(instance, local) {
...
.. warning::
    ...
    "developer mode": log into your inCore instance (user *admin* password
    *admin* by default) open the user menu (in the top-right corner of the
    inCore screen) and select :guilabel:`About inCore` then :guilabel:`Activate
    the developer mode`:

    .. image:: web/about_incore.png
        :align: center
...
The inCore framework uses one such pattern to define modules within web addons,
...
In inCore web, modules are declared as functions set on the global ``incore``
...
* the first parameter is the current instance of the inCore web client, it gives
  access to various capabilities defined by the inCore (translations,
...
* the second parameter is your own local namespace automatically created by
  ...
  outside your module (either because the inCore web client needs to call them
...
For simplicity and developer-friendliness inCore web provides a class
system based on John Resig's `Simple JavaScript Inheritance`_.

New classes are defined by calling the :func:`~incore.web.Class.extend`
method of :class:`incore.web.Class`::
...
The :func:`~incore.web.Class.extend` method takes a dictionary describing
...
by calling :func:`~incore.web.Class.extend` on the parent class, as is done
to subclass :class:`~incore.web.Class`::
...
The inCore web client bundles jQuery_ for easy DOM manipulation. It is useful
...
inCore Web makes specific components responsible for sections of a page. In
inCore web, the base for such components is the :class:`~incore.Widget`
...
It extends :class:`~incore.Widget` and overrides the standard method
:func:`~incore.Widget.start`, which — much like the previous ``MyClass``
— does little for now.
...
The ``HomePage`` widget already has a :func:`~incore.Widget.start`
...
All widgets have a :attr:`~incore.Widget.$el` which represents the
section of page they're in charge of (as a jQuery_ object). Widget content
should be inserted there. By default, :attr:`~incore.Widget.$el` is an
empty ``<div>`` element.
...
Let's add some content to the widget's root element, using jQuery::
    ...
    local.HomePage = instance.Widget.extend({
        start: function() {
            this.$el.append("<div>Hello dear inCore user!</div>");
        },
    });
...
.. note::
    ...
    to refresh the javascript code loaded in inCore Web, you will need to reload
    the page. There is no need to restart the inCore server.

The ``HomePage`` widget is used by inCore Web and managed automatically.
...
``GreetingsWidget``'s :func:`~incore.Widget.appendTo` method::
...
* Finally it tells ``GreetingsWidget`` where to insert itself, delegating part
  of its :attr:`~incore.Widget.$el` to the ``GreetingsWidget``.

When the :func:`~incore.Widget.appendTo` method is called, it asks the
widget to insert itself at the specified position and to display its content.
The :func:`~incore.Widget.start` method will be called during the call
to :func:`~incore.Widget.appendTo`.
...
.. code-block:: html
    ...
    <div class="oe_petstore_homepage">
        <div>Hello dear inCore user!</div>
...
:class:`~incore.Widget`, because we added some classes on them.
...
:func:`~incore.Widget.getChildren`
...
When overriding the :func:`~incore.Widget.init` method of a widget it is
...
it. This is done via the :func:`~incore.Widget.destroy` method::

    greeting.destroy();

When a widget is destroyed it will first call
:func:`~incore.Widget.destroy` on all its children. Then it erases itself
from the DOM. If you have set up permanent structures in
:func:`~incore.Widget.init` or :func:`~incore.Widget.start` which
must be explicitly cleaned up (because the garbage collector will not handle
them), you can override :func:`~incore.Widget.destroy`.

.. danger::

    when overriding :func:`~incore.Widget.destroy`, ``_super()``
...
As many other environments, inCore's solution is to use a `template engine`_.
inCore's template engine is called :ref:`reference/qweb`.
...
* It has special support in inCore Web's :class:`~incore.Widget`, though it
  can be used outside of inCore's web client (and it's possible to use
  :class:`~incore.Widget` without relying on QWeb)
...
.. note::
    ...
    like inCore :ref:`views <reference/views>`.
    ...
    characteristic of inCore, and losing it was considered unacceptable.
...
However, because :class:`~incore.Widget` has special integration for QWeb
the template can be set directly on the widget via its
:attr:`~incore.Widget.template` attribute::
...
* with the second version, the template is rendered right before
  :func:`~incore.Widget.start` is called
...
.. warning::
    ...
    they're set as a widget's :attr:`~incore.Widget.template`. If there are
...
When using :class:`~incore.Widget`'s integration it is not possible to
...
before :func:`~incore.Widget.start` is called (the widget's state will
essentially be that set up by :func:`~incore.Widget.init`):
...
But because it's a common operation, :class:`~incore.Widget` provides an
equivalent shortcut through the :func:`~incore.Widget.$` method::
...
:attr:`~incore.Widget.events` is an object (mapping) of an event to the
...
:func:`~incore.Widget.trigger` takes the name of the event to trigger as
...
listening to the ``user_chose`` event using :func:`~incore.Widget.on`::
...
:func:`~incore.Widget.on` binds a function to be called when the
...
additional arguments of :func:`~incore.Widget.trigger` if it has
any. Example::
...
.. note::
    ...
    exception to that rule is ``incore.web.bus`` which exists specifically
    to broadcasts evens in which any widget could be interested throughout
    the inCore web application.
...
* :func:`~incore.Widget.set` sets the value of a property and triggers
  :samp:`change:{propname}` (where *propname* is the property name passed as
  first parameter to :func:`~incore.Widget.set`) and ``change``
* :func:`~incore.Widget.get` retrieves the value of a property.
...
The class system of the inCore web framework allows direct modification of
existing classes using the :func:`~incore.web.Class.include` method::
...
defined in the call to :func:`~incore.web.Class.include`. This will also work
if some instances of the class (or of any of its sub-classes) were created
prior to the call to :func:`~incore.Widget.include`.
...
In inCore, translations files are automatically generated by scanning the source
...
:func:`~incore.web._t` (and also :func:`~incore.web._lt`).
...
:func:`~incore.web._lt` ("lazy translate") is similar but somewhat more
...
Communication with the inCore Server
==================================

Contacting Models
-----------------

Most operations with inCore involve communicating with *models* implementing
...
communicating with inCore requires additional metadata whose setup before every
call would be verbose and error-prone. As a result, inCore web provides
...
The class used to call inCore models is :class:`incore.Model`. It is
instantiated with the inCore model's name as first parameter
(``oepetstore.message_of_the_day`` here).

:func:`~incore.web.Model.call` can be used to call any (public) method of an
inCore model. It takes the following positional arguments:
...
:func:`~incore.Widget.call` returns a deferred resolved with the value
...
the time zone of the user, used to compute correctly dates and times if inCore
...
solution to achieve that is to use :class:`incore.web.CompoundContext`.

:class:`~incore.web.CompoundContext` is a class used to pass the user's
...
dictionaries or other :class:`~incore.web.CompoundContext` instances. It will
...
are related to the configuration of the current user in inCore plus the
``new_key`` key that was added when instantiating
:class:`~incore.web.CompoundContext`.

Queries
-------

While :func:`~incore.Model.call` is sufficient for any interaction with inCore
models, inCore Web provides a helper for simpler and clearer querying of models
(fetching of records based on various conditions):
:func:`~incore.Model.query` which acts as a shortcut for the common
combination of :py:meth:`~incore.models.Model.search` and
::py:meth:`~incore.models.Model.read`. It provides a clearer syntax to search
and read models::
...
* :func:`~incore.web.Model.query` takes an optional list of fields as
  parameter (if no field is provided, all fields of the model are fetched). It
  returns a :class:`incore.web.Query` which can be further customized before
  being executed
* :class:`~incore.web.Query` represents the query being built. It is
  ...
  :class:`~incore.web.Query` for its customization options.

When the query is set up as desired, simply call
:func:`~incore.web.Query.all` to execute it and return a
deferred to its result. The result is the same as
:py:meth:`~incore.models.Model.read`'s, an array of dictionaries where each
...
.. exercise:: Pet Toys List
    ...
    In inCore, images are generally stored in regular fields encoded as
...
In inCore, many operations start from an :ref:`action <reference/actions>`:
...
In inCore Web, the component responsible for handling and reacting to these
...
:func:`~incore.Widget.do_action` is a shortcut of :class:`~incore.Widget`
...
the inCore web know to start this widget? Because the widget is registered as
a *client action*.

A client action is (as its name implies) an action type defined almost
entirely in the client, in javascript for inCore web. The server simply sends
...
``instance.web.client_actions`` is a :class:`~incore.web.Registry` in which
the action manager looks up client action handlers when it needs to execute
one. The first parameter of :class:`~incore.web.Registry.add` is the name
(tag) of the client action, and the second parameter is the path to the widget
from the inCore web client root.
...
Much of inCore web's usefulness (and complexity) resides in views. Each view
...
Most :ref:`inCore views <reference/views>` are implemented through a subclass
of :class:`incore.web.View` which provides a bit of generic basic structure
for handling events and displaying model information.

The *search view* is considered a view type by the main inCore framework, but
...
:py:class:`~incore.models.Model.fields_view_get`) and any other data source
it needs. To that purpose, views are provided with an optional view
identifier set as the :attr:`~incore.web.View.view_id` attribute.

Views are also provided with a :class:`~incore.web.DataSet` instance which
...
:func:`~incore.web.View.do_search`, and updating their
:class:`~incore.web.DataSet` as necessary.
...
:py:class:`~incore.fields.Char` fields which should contain email addresses
...
* It must correctly implement the 3 field attributes available in all fields
  of inCore. The ``AbstractField`` class already implements an algorithm that
  ...
  * ``readonly``: When ``true``, the field must not be editable by the
    user. Most fields in inCore have a completely different behavior depending
...
* Fields have two methods, ``set_value()`` and ``get_value()``, which are
  ...
  given by the inCore server when a ``read()`` is performed on a model and give
  ...
  ``write()`` is not necessarily the same in inCore. As example, when you read a
...
interface and the ``AbstractField`` class directly in the code of the inCore web
client.
...
user to modify it can be useful, but most fields in inCore also allow editing.
...
their :attr:`~incore.web.form.FormWidget.field_manager` attribute::
...
:attr:`~incore.web.form.FormWidget` is generally the
:class:`~incore.web.form.FormView` itself, but features used from it should
be limited to those defined by :class:`~incore.web.form.FieldManagerMixin`,
the most useful being:

* :func:`~incore.web.form.FieldManagerMixin.get_field_value(field_name)`
  which returns the value of a field.
* :func:`~incore.web.form.FieldManagerMixin.set_values(values)` sets multiple
