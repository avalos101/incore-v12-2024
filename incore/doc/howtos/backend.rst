...
.. warning::
    ...
    This tutorial requires :ref:`having installed inCore <setup/install>`

Start/Stop the inCore server
==========================

inCore uses a client/server architecture in which clients are web browsers
accessing the inCore server via RPC.
...
In order to start the server, simply invoke the command :ref:`incore-bin
...
Build an inCore module
====================

Both server and client extensions are packaged as *modules* which are
optionally loaded in a *database*.

inCore modules can either add brand new business logic to an inCore system, or
alter and extend existing business logic: a module can be created to add your
country's accounting rules to inCore's generic accounting support, while the
next module adds support for real-time visualisation of a bus fleet.

Everything in inCore thus starts and ends with modules.

Composition of a module
-----------------------

An inCore module can contain a number of elements:

Business objects
    Declared as Python classes, these resources are automatically persisted
    by inCore based on their configuration
...
are specified by using the :option:`--addons-path <incore-bin --addons-path>`
option.
...
An inCore module is declared by its :ref:`manifest <reference/module/manifest>`.
...
inCore provides a mechanism to help set up a new module, :ref:`incore-bin
<reference/cmdline/server>` has a subcommand :ref:`scaffold
<reference/cmdline/scaffold>` to create an empty module:

.. code-block:: console

    $ incore-bin scaffold <module name> <where to put it>
...
.. exercise:: Module creation
    ...
    install it in inCore.

    .. only:: solutions

        #. Invoke the command ``incore-bin scaffold openacademy addons``.
...
A key component of inCore is the :abbr:`ORM (Object-Relational Mapping)` layer.
...
:class:`~incore.models.Model` which integrates them into the automated
persistence system.

Models can be configured by setting a number of attributes at their
definition. The most important attribute is
:attr:`~incore.models.Model._name` which is required and defines the name for
the model in the inCore system. Here is a minimally complete definition of a
model::
...
:attr:`~incore.fields.Field.string` (``unicode``, default: field's name)
    The label of the field in UI (visible by users).
:attr:`~incore.fields.Field.required` (``bool``, default: ``False``)
    If ``True``, the field can not be empty, it must either have a default
    value or always be given a value when creating a record.
:attr:`~incore.fields.Field.help` (``unicode``, default: ``''``)
    Long-form, provides a help tooltip to users in the UI.
:attr:`~incore.fields.Field.index` (``bool``, default: ``False``)
    Requests that inCore create a `database index`_ on the column.
...
Example of simple fields are :class:`~incore.fields.Boolean`,
:class:`~incore.fields.Date`, :class:`~incore.fields.Char`.

Reserved fields
###############

inCore creates a few fields in all models\ [#autofields]_. These fields are
managed by the system and shouldn't be written to. They can be read if
useful or necessary:

:attr:`~incore.fields.Model.id` (:class:`~incore.fields.Id`)
    The unique identifier for a record in its model.
:attr:`~incore.fields.Model.create_date` (:class:`~incore.fields.Datetime`)
    Creation date of the record.
:attr:`~incore.fields.Model.create_uid` (:class:`~incore.fields.Many2one`)
    User who created the record.
:attr:`~incore.fields.Model.write_date` (:class:`~incore.fields.Datetime`)
    Last modification date of the record.
:attr:`~incore.fields.Model.write_uid` (:class:`~incore.fields.Many2one`)
    user who last modified the record.

Special fields
##############

By default, inCore also requires a ``name`` field on all models for various
display and search behaviors. The field used for these purposes can be
overridden by setting :attr:`~incore.models.Model._rec_name`.
...
inCore is a highly data driven system. Although behavior is customized using
Python_ code part of a module's value is in the data it sets up when loaded.

.. tip:: some modules exist solely to add data into inCore
    :class: aphorism
...
* ``model`` is the name of the inCore model for the record.
...
.. tip:: The content of the data files is only loaded when a module is
    ...
    :ref:`incore-bin -u openacademy <reference/cmdline>` to save the changes
    to your database.
...
If no search view exists for the model, inCore generates one which only allows
...
:class:`Many2one(other_model, ondelete='set null') <incore.fields.Many2one>`
...
:class:`One2many(other_model, related_field) <incore.fields.One2many>`
    A virtual relationship, inverse of a :class:`~incore.fields.Many2one`.
    A :class:`~incore.fields.One2many` behaves as a container of records,
    ...
    .. danger::
        ...
        Because a :class:`~incore.fields.One2many` is a virtual relationship,
        there *must* be a :class:`~incore.fields.Many2one` field in the
        :samp:`{other_model}`, and its name *must* be :samp:`{related_field}`

:class:`Many2many(other_model) <incore.fields.Many2many>`
...
inCore provides two *inheritance* mechanisms to extend an existing model in a
modular way.
...
.. seealso::
    ...
    * :attr:`~incore.models.Model._inherit`
    * :attr:`~incore.models.Model._inherits`

View inheritance
----------------

Instead of modifying existing views in place (by overwriting them), inCore
...
In inCore, :ref:`reference/orm/domains` are values that encode conditions on
...
:attr:`~incore.fields.Field.compute` to the name of a method. The computation
...
on the compute method with the decorator :func:`~incore.api.depends`.
    ...
    * Add a field ``active`` in the class Session, and set sessions as active by
    ...
    .. only:: solutions
        ...
        .. note::
            ...
            inCore has built-in rules making fields with an ``active`` field set
            to ``False`` invisible.
...
the record in the form view, and decorate it with :func:`~incore.api.onchange`
...
inCore provides two ways to set up automatically verified invariants:
:func:`Python constraints <incore.api.constrains>` and
:attr:`SQL constraints <incore.models.Model._sql_constraints>`.

A Python constraint is defined as a method decorated with
:func:`~incore.api.constrains`, and invoked on a recordset. The decorator
...
:attr:`~incore.models.Model._sql_constraints`. The latter is assigned to a list
...
.. exercise:: Add access control through the inCore interface
...
the same convention as the method :meth:`~incore.models.Model.write` of the ORM.
...
:class:`~incore.models.TransientModel` instead of
:class:`~incore.models.Model`. The class
:class:`~incore.models.TransientModel` extends :class:`~incore.models.Model`
...
pt_BR.po). Translations will be loaded automatically by inCore for all
enabled languages. Developers always use English when creating a module, then
export the module terms using inCore's gettext POT export feature
...
.. tip:: The Portable Object files generated by inCore are published on
         `Transifex <https://www.transifex.com/incore/public/>`__, making it
...
.. code-block:: text
   ...
   |- idea/ # The module directory
      |- i18n/ # Translation files
         | - idea.pot # Translation Template (exported from inCore)
...
.. tip:: 
   ...
   By default inCore's POT export only extracts labels inside XML files or
   inside field definitions in Python code, but any Python string can be
   translated this way by surrounding it with the function :func:`incore._`
   (e.g. ``_("Label")``)

.. exercise:: Translate a module

   Choose a second language for your inCore installation. Translate your
   module using the facilities provided by inCore.
   ...
   .. only:: solutions
        ...
        #. In ``models.py``, add an import statement for the function
           ``incore._`` and mark missing strings as translatable
...
inCore uses a report engine based on :ref:`reference/qweb`,
...
http://localhost:8080/report/html/account.report_invoice/1 (if ``account`` is
installed) and the PDF version through
http://localhost:8080/report/pdf/account.report_invoice/1.
...
.. danger::
    ...
    - ``report.url``, pointing to an URL reachable from your server
      (probably ``http://localhost:8080`` or something similar). It will be
...
inCore is accessible through XML-RPC/JSON-RPC interfaces, for which libraries
exist in many languages.

XML-RPC Library
---------------

The following example is a Python 3 program that interacts with an inCore
...
.. exercise:: Add a new service to the client
   ...
   inCore (yours, or your instructor's). This program should display all
...
The following example is a Python 3 program that interacts with an inCore server
...
.. note::
    ...
    There are a number of high-level APIs in various languages to access inCore
    systems without *explicitly* going through XML-RPC or JSON-RPC, such as:

    * https://github.com/akretion/ooor
    * https://github.com/OCA/incorerpc
    * https://github.com/nicolas-van/incore-client-lib
    * http://pythonhosted.org/inCoreRPC
    * https://github.com/abhishek-jaiswal/php-incore-lib

.. [#autofields] it is possible to :attr:`disable the automatic creation of some
                 fields <incore.models.Model._log_access>`
.. [#rawsql] writing raw SQL queries is possible, but requires care as it
             bypasses all inCore authentication and security mechanisms.
