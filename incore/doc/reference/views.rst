...
``groups_id``
    :class:`~incore.fields.Many2many` field to the groups allowed to view/use
    the current view
``inherit_id``
...
``banner_route``
    ...
    .. code-block:: python
        ...
        class MyController(incore.http.Controller):
...
``button``
    ...
    ``type``
        type of button, indicates how it clicking it affects inCore:
    ...
    ``context``
        merged into the view's context when performing the button's inCore call
    ``confirm``
        confirmation message to display (and for the user to accept) before
        performing the button's inCore call
...
Semantic components tie into and allow interaction with the inCore
system. Available semantic components are:

``button``
  call into the inCore system, similar to :ref:`list view buttons
...
``field``
  ...
  ``widget``
    ...
    (e.g. :class:`~incore.fields.Char`,
    :class:`~incore.fields.Many2one`). The ``widget`` attributes allows using
  ...
  ``on_change``
    ...
    .. deprecated:: 8.0
       ...
       Use :func:`incore.api.onchange` on the model
  ...
  ``mode``
    for :class:`~incore.fields.One2many`, display mode (view type) to use for
  ...
  ``filename``
    ...
    file
  ``password``
    indicates that a :class:`~incore.fields.Char` field stores a password and
...
Most :class:`~incore.fields.Many2many` fields, like categories, are better
...
``templates``
  ...
  ``record``
    ...
    a :meth:`~incore.models.Model.read` (except for date and datetime fields
    that are `formatted according to user's locale
    <https://github.com/incore/incore/blob/a678bd4e/addons/web_kanban/static/src/js/kanban_record.js#L102>`_)
  ``read_only_mode``
    self-explanatory
    ...
    * buttons and links with a ``type`` attribute become perform inCore-related
      operations rather than their standard HTML function. Possible types are:

      ``action``, ``object``
        standard behavior for :ref:`inCore buttons
        <reference/views/list/button>`, most attributes relevant to standard
        inCore buttons can be used.
      ``open``
...
``arrow`` (required, 1)
    ...
    ``object`` (required)
      the edge's inCore model
    ``source`` (required)
      :class:`~incore.fields.Many2one` field of the edge's model pointing to
...
.. warning::
   ...
   The Dashboard view is only available in inCore Enterprise.
...
``aggregate``
    ...
    - ``group_operator`` (optional)
        ...
        .. note:: The special aggregate function ``count_distinct`` (defined in incore) can also be used here
...
.. warning::
   ...
   The Cohort view is only available in inCore Enterprise.
...
``field``
    ...
    ``widget``
        ...
        standard inCore 8.0 is a ``selection`` widget for
        :class:`~incore.fields.Many2one` fields)
    ``domain``
        if the field can provide an auto-completion
        (e.g. :class:`~incore.fields.Many2one`), filters the possible
        completion results.
...
``filter``
    ...
    ``domain`` (optional)
        an inCore :ref:`domain <reference/orm/domains>`, will be appended to the
