...
This document presents the inCore Javascript framework. This
...
.. note:: An html version of all docstrings in inCore is available at:
...
- the *website*: this is the public part of inCore.  It allows an unidentified
...
It means that while a user is working on inCore, the web client class (and the
...
Managing assets in inCore is not as straightforward as it is in some other apps.
...
a collection of files (javascript, css, scss). In inCore, the most important
...
Main bundles
------------
When the inCore server is started, it checks the timestamp of each file in a bundle,
...
- *web.assets_common*: this bundle contains most assets which are common to the
  ...
  lower level building blocks for the incore framework.  Note that it contains the
  *boot.js* file, which defines the incore module system.
...
.. note ::
    ...
    incore web client.  This means that the files are transferred through the network
...
sure they are loaded in the correct order.  In order to do that, inCore has defined
a small module system (located in the file *addons/web/static/src/js/boot.js*,
which needs to be loaded first).

The inCore module system, inspired by AMD, works by defining the function *define*
on the global incore object. We then define each javascript module by calling that
function.  In the inCore framework, a module is a piece of code that will be executed
...
.. code-block:: javascript
    ...
    // in file a.js
    incore.define('module.A', function (require) {
        "use strict";
    ...
    // in file b.js
    incore.define('module.B', function (require) {
        "use strict";
...
.. code-block:: javascript
    ...
    incore.define('module.Something', ['module.A', 'module.B'], function (require) {
        "use strict";
...
The *incore.define* method is given three arguments:

- *moduleName*: the name of the javascript module.  It should be a unique string.
  The convention is to have the name of the incore addon followed by a specific
...
.. code-block:: javascript
    ...
    incore.define('module.Something', ['web.ajax'], function (require) {
        "use strict";
...
inCore was developed before ECMAScript 6 classes were available.  In Ecmascript 5,
...
For these reasons, inCore decided to use its own class system, inspired by John
...
The incore Class system does not support multiple inheritance, but for those cases
...
the way inCore is structured, it is sometimes necessary in one addon to modify
...
* Global selectors should be avoided. Because a component may be used several
  times in a single page (an example in inCore is dashboards), queries should be
  ...
  lead to unintended or incorrect behavior.  inCore Web's
...
There are currently two event systems supported by inCore: a simple system which
...
- *trigger_up*: this is the method that will create a small *inCoreEvent* and
...
The inCoreEvent class is very simple.  It has three public attributes: *target*
...
A common need in the inCore ecosystem is to extend/change the behaviour of the
...
There are typically two usecases when working on inCore: one may need to call a
...
The inCore framework has a standard way to communicate various informations to the
...
The notification system in inCore is designed with the following components:
...
In 2017, inCore replaced the previous view code with a new architecture.  The
...
handled by incore and correspond to italic and bold, respectively).  Note that the
