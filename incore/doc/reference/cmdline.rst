...
================================
Command-line interface: incore-bin
================================
...
.. option:: -s, --save
    ...
    (:file:`{$HOME}/.incorerc` by default, and can be overridden using
    :option:`-c`)
...
.. option:: --db-filter <filter>
    ...
    - ``%d`` is replaced by the subdomain the request is made on, with the
      exception of ``www`` (so domain ``incore.co`` and ``www.incore.co`` both
      match the database ``incore``).

      These operations are case sensitive. Add option ``(?i)`` to match all
      databases (so domain ``incore.co`` using ``(?i)%d`` matches the database
      ``inCore``).
    ...
    .. code-block:: bash
        ...
        incore-bin --db-filter ^11.*$

    Restrict access to databases whose name starts with 11

    .. code-block:: bash

        incore-bin --database 11firstdatabase,11seconddatabase

    Restrict access to only two databases, 11firstdatabase and 11seconddatabase
    
    .. code-block:: bash

        incore-bin --database 11firstdatabase,11seconddatabase -u base
    ...
    .. code-block:: bash
        ...
        incore-bin --db-filter ^11.*$ --database 11firstdatabase,11seconddatabase -u base
...
.. option:: --db_sslmode
    ...
    Control the SSL security of the connection between inCore and PostgreSQL.
...
Use these options to translate inCore to another language. See i18n section of
...
By default, inCore displays all logging of level_ ``info`` except for workflow
...
.. option:: --log-handler <handler-spec>
    ...
    e.g. ``incore.models:DEBUG`` will enable all logging messages at or above
    ...
    * If the level is omitted, the logger is set to ``INFO``
    ...
    .. code-block:: console
        ...
        $ incore-bin --log-handler :DEBUG --log-handler werkzeug:CRITICAL --log-handler incore.fields:WARNING

.. option:: --log-request

    enable DEBUG logging for RPC requests, equivalent to
    ``--log-handler=incore.http.rpc.request:DEBUG``

.. option:: --log-response

    enable DEBUG logging for RPC responses, equivalent to
    ``--log-handler=incore.http.rpc.response:DEBUG``

.. option:: --log-web

    enables DEBUG logging of HTTP requests and responses, equivalent to
    ``--log-handler=incore.http:DEBUG``

.. option:: --log-sql

    enables DEBUG logging of SQL querying, equivalent to
    ``--log-handler=incore.sql_db:DEBUG``
...
.. option:: --log-level <level>
    ...
    ``incore`` and ``werkzeug`` loggers (except for ``debug`` which is only
    set on ``incore``).

    inCore also provides debugging pseudo-levels which apply to different sets
    of loggers:
    ...
    ``debug_rpc``
        sets the ``incore`` and HTTP request loggers to ``debug``

        equivalent to ``--log-level debug --log-request``
    ``debug_rpc_answer``
        sets the ``incore`` and HTTP request and response loggers to
        ``debug``
...
.. option:: --email-from <address>
    ...
    Email address used as <FROM> when inCore needs to send mails
...
.. option:: --smtp-ssl
    ...
    If set, incore should use SSL/STARTSSL SMTP connections
...
.. program:: incore-bin scaffold

Scaffolding is the automated creation of a skeleton structure to simplify
bootstrapping (of new modules, in the case of inCore). While not necessary it
avoids the tedium of setting up basic structures and looking up what all
starting requirements are.

Scaffolding is available via the :command:`incore-bin scaffold` subcommand.
...
The default configuration file is :file:`{$HOME}/.incorerc` which
can be overridden using :option:`--config <incore-bin -c>`. Specifying
:option:`--save <incore-bin -s>` will save the current configuration state back
to that file.
