...
QWeb is the primary templating_ engine used by inCore\ [#othertemplates]_. It
...
:meth:`incore.http.HttpRequest.render`:
...
This automatically creates a :class:`~incore.http.Response` object which can
...
.. py:method:: render(cr, uid, id[, values][, engine='ir.qweb][, context])
    ...
    ``request``
        the current :class:`~incore.http.WebRequest` object, if any
    ``debug``
    ...
    :param str engine: name of the inCore model to use for rendering, can be
...
.. .. automodule:: incore.addons.base.ir.ir_qweb
...
.. js:class:: QWeb2.Engine
    ...
    inCore Web instantiates one for the user in the core module, and
    ...
    inCore Web, Kanban views get their own :js:class:`QWeb2.Engine`
    ...
    .. js:attribute:: QWeb2.Engine.preprocess_node
        ...
        node to template code. In inCore Web, this is used to
...
.. [#othertemplates] although it uses a few others, either for historical
                     ...
                     use case. inCore 9.0 still depends on Jinja_ and Mako_.
