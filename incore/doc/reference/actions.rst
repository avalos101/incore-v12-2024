...
``views``
    ...
    :meth:`~incore.models.Model.fields_view_get`). The first type of the
...
Allow opening a URL (website/web page) via an inCore action. Can be customized
via two fields:
...
will replace the current content section by the inCore home page.
...
``model_id``
    inCore model linked to the action, made available in
...
Creates a new record, from scratch (via :meth:`~incore.models.Model.create`)
or by copying an existing record (via :meth:`~incore.models.Model.copy`)
...
``fields_lines``
    ...
    :class:`~incore.fields.One2many` with the fields:
...
``crud_model_id``
    ...
    ``new_other``
``ref_object``
    :class:`~incore.fields.Reference` to an arbitrary record to copy, used if
...
``groups_id``
    ...
    the current report
``paperformat_id``
    :class:`~incore.fields.Many2one` field to the paper format you wish to
