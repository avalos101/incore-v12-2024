...
There are many ways to test an application.  In inCore, we have three kinds of
tests
...
inCore provides support for testing modules using unittest.
...
`unittest documentation`_, but inCore provides a number of utilities and helpers
related to testing inCore content (modules, mainly):

.. autoclass:: incore.tests.common.TransactionCase
    :members: browse_ref, ref

.. autoclass:: incore.tests.common.SingleTransactionCase
    :members: browse_ref, ref

.. autoclass:: incore.tests.common.SavepointCase

.. autoclass:: incore.tests.common.HttpCase
    :members: browse_ref, ref, url_open, phantom_js

.. autofunction:: incore.tests.common.tagged
...
.. autofunction:: incore.tests.common.at_install

.. autofunction:: incore.tests.common.post_install

The most common situation is to use
:class:`~incore.tests.common.TransactionCase` and test a property of a model
in each method::
...
.. autoclass:: incore.tests.common.Form
    :members:

.. autoclass:: incore.tests.common.M2MProxy
    :members: add, remove, clear

.. autoclass:: incore.tests.common.O2MProxy
...
:option:`--test-enable <incore-bin --test-enable>` was enabled when starting the
inCore server.

As of inCore 8, running tests outside of the install/update cycle is not
supported.
...
In inCore, Python tests can be tagged to facilitate the test selection when
running tests.

Subclasses of :class:`incore.tests.common.BaseCase` (usually through
:class:`~incore.tests.common.TransactionCase`,
:class:`~incore.tests.common.SavepointCase` or
:class:`~incore.tests.common.HttpCase`) are automatically tagged with
``standard``, ``at_install`` and their source module's name by default.

Invocation
^^^^^^^^^^

:option:`--test-tags <incore-bin --test-tags>` can be used to select/filter tests
to run on the command-line.

This option defaults to ``+standard`` meaning tests tagged ``standard``
(explicitly or implicitly) will be run by default when starting inCore
with :option:`--test-enable <incore-bin --test-enable>`.

When writing tests, the :func:`~incore.tests.common.tagged` decorator can be
used on **test classes** to add or remove tags.

The decorator's arguments are tag names, as strings.

.. danger:: :func:`~incore.tests.common.tagged` is a class decorator, it has no
...
.. code-block:: python
    ...
    from incore.tests import TransactionCase, tagged
...
.. code-block:: console
    ...
    $ incore-bin --test-enable --test-tags nice
...
:option:`--test-tags <incore-bin --test-tags>`: on the command-line, values
are *additive* (you're selecting all tests with *any* of the specified tags)

.. code-block:: console

    $ incore-bin --test-enable --test-tags nice,standard
...
.. code-block:: console
    ...
    $ incore-bin --test-enable --test-tags 'standard,-slow'

When you write a test that does not inherit from the
:class:`~incore.tests.common.BaseCase`, this test will not have the default tags,
...
- ``standard``: All inCore tests that inherit from
  :class:`~incore.tests.common.BaseCase` are implicitely tagged standard.
  :option:`--test-tags <incore-bin --test-tags>` also defaults to ``standard``.
...
- *module_name*: inCore tests classes extending
  :class:`~incore.tests.common.BaseCase` are implicitely tagged with the
  ...
  .. code-block:: console
      ...
      $ incore-bin --test-enable --test-tags stock_account
...
.. important::
    ...
    modules have to be selected with the :option:`-u <incore-bin -u>` or
    :option:`-i <incore-bin -i>` switches.  For simplicity, those switches are
...
.. code-block:: console
    ...
    $ incore-bin --test-enable --test-tags sale

Run the tests from the sale module but not the ones tagged as slow:

.. code-block:: console

    $ incore-bin --test-enable --test-tags 'sale,-slow'

Run only the tests from stock or tagged as slow:

.. code-block:: console

    $ incore-bin --test-enable --test-tags '-standard, slow, stock'
...
inCore Web includes means to unit-test both the core code of
inCore Web and your own javascript modules. On the javascript side,
unit-testing is based on QUnit_ with a number of helpers and
extensions for better integration with inCore.

To see what the runner looks like, find (or start) an inCore server
