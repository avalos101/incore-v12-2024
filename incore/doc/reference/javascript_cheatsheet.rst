...
There are many ways to solve a problem in JavaScript, and in inCore.  However, the
inCore framework was designed to be extensible (this is a pretty big constraint),
and some common problems have a nice standard solution.  The standard solution
has probably the advantage of being easy to understand for an incore developers,
and will probably keep working when inCore is modified.
...
First of all, remember that the first rule of customizing incore with JS is:
...
example, the voip addon in incore need to modify the FieldPhone widget to add the
