...
automatically loaded by inCore when the corresponding language is installed (via
...
inCore automatically exports translatable strings from "data"-type content:
...
* QWeb templates (both server-side and client-side), all text nodes are
  ...
  exported
* for :class:`~incore.fields.Field`, unless their model is marked with
  ``_translate = False``:
...
* help/error messages of :attr:`~incore.models.Model._constraints` and
  :attr:`~incore.models.Model._sql_constraints` are exported

Explicit exports
================

When it comes to more "imperative" situations in Python code or Javascript
code, inCore cannot automatically export translatable terms so they
must be marked explicitly for export. This is done by wrapping a literal
string in a function call.

In Python, the wrapping function is :func:`incore._`::

    title = _("Bank Accounts")

In JavaScript, the wrapping function is generally :js:func:`incore.web._t`:
