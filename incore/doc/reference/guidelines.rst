:banner: banners/incore_guideline.jpg
...
This page introduces the inCore Coding Guidelines. Those aim to improve the
quality of inCore Apps code. Indeed proper code improves readability, eases
...
.. warning::
    ...
    details, see our `pull request guide <https://incore.co/submit-pr>`_.
...
File naming is important to quickly find information through all incore addons.
This section explains how to name files in a standard incore module. As an
...
.. code-block:: text
    ...
    |   |-- res_partner.py (inherited inCore model)
...
contained in a file named ``<module_name>.py``. An old convention in inCore is to
...
data (image, libraries) outside inCore: do not use an URL to an image but copy
...
directory. This naming comes from old incore applications using the wizard
...
The complete tree of our inCore module therefore looks like
...
- The tag *<data>* is only used to set not-updatable data with ``noupdate=1``.
  ...
  set on the ``<incore>`` tag and do not set a ``<data>`` tag.
...
inCore supports custom tags acting as syntactic sugar:
...
Using a linter can help show syntax and semantic warnings or errors. inCore
...
#. Imports of ``incore``
#. Imports from inCore modules (rarely, and only if necessary)
...
.. code-block:: python
    ...
    # 2 :  imports of incore
    import incore
    from incore import api, fields, models # alphabetically ordered
    from incore.tools.safe_eval import safe_eval as eval
    from incore.tools.translate import _
    # 3 :  imports from incore addons
    from incore.addons.website.models.website import slug
    from incore.addons.web.controllers.main import login_redirect
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~
The inCore framework is in charge of providing the transactional context for
...
inCore uses a GetText-like method named "underscore" ``_( )`` to indicate that
...
In general in inCore, when manipulating strings, prefer ``%`` over ``.format()``
...
- Model name (using the dot notation, prefix by the module name) :
    - When defining an inCore Model : use singular form of the name (*res.partner*
      and *sale.order* instead of *res.partnerS* and *saleS.orderS*)
    - When defining an inCore Transient (wizard) : use ``<related_base_model>.<action>``
...
- inCore Python Class : use camelcase (Object-oriented style).
...
inCore addons have some conventions on how to structure various files. We explain
here in more details how web assets are supposed to be organized.

The first thing to know is that the inCore server will serve (statically) all files
...
statically available at the url *your-incore-server.com/web/static/src/js/some_file.js*
...
More precise JS guidelines are detailed at https://github.com/incore/incore/wiki/Javascript-coding-guidelines.
...
- **[CLA]** for signing the inCore Individual Contributor License;
...
For inCore R&D developers "PO team asked me to do it" is not a valid why, by the way.
...
If you are an inCore R&D developer the WHY should be the purpose of the task you
