...
inCore is greatly data-driven, and a big part of modules definition is thus
...
The main way to define data in inCore is via XML data files: The broad structure
of an XML data file is the following:

* Any number of operation elements within the root element ``incore``
...
``search``
    ...
    the field is a :class:`~incore.fields.Many2one`
``ref``
    ...
    Mostly for :class:`~incore.fields.Many2one` and
    :class:`~incore.fields.Reference` fields
``type``
...
Because some important structural models of inCore are complex and involved,
...
.. literalinclude:: ../../incore/addons/base/data/res.country.state.csv
    :language: text
    :lines: 1-15
...
.. csv-table::
    :file: ../../incore/addons/base/data/res.country.state.csv
    :header-rows: 1
