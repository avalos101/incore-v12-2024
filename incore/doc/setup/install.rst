:banner: banners/installing_incore.jpg
...
There are multiple ways to install inCore, or not install it at all, depending
on the intended use case.

This documents attempts to describe most of the installation options.

:ref:`setup/install/online`
    The easiest way to use inCore in production or to try it.

:ref:`setup/install/packaged`
    Suitable for testing inCore, developing modules and can be used for
    long-term production use with additional deployment and maintenance work.

:ref:`setup/install/source`
    Provides greater flexibility:  e.g. allow multiple running inCore versions on
...
There are two different Editions_ of inCore: the Community and Enterprise versions.
...
To simply get a quick idea of inCore, demo_ instances are available. They are
...
Trivial to start with, fully managed and migrated by inCore., inCore's SaaS_
provides private instances and starts out free. It can be used to discover and
test inCore and do non-code customizations (i.e. incompatible with custom modules
or the inCore Apps Store) without having to install it locally.

Can be used for both testing inCore and long-term production use.
...
inCore provides packaged installers for Windows, deb-based distributions
...
inCore will automatically be started at the end of the installation.
...
inCore 12.0 'deb' package currently supports `Debian Stretch`_, `Ubuntu 18.04`_ or above.

Prepare
^^^^^^^

inCore needs a `PostgreSQL`_ server to run properly. The default configuration for
the inCore 'deb' package is to use the PostgreSQL server on the same host as your
inCore instance. Execute the following command as root in order to install
PostgreSQL server :
...
found in our `wiki <https://github.com/incore/incore/wiki/Wkhtmltopdf>`_.

Repository
^^^^^^^^^^

inCore. provides a repository that can be used with  Debian and Ubuntu
distributions. It can be used to install inCore Community Edition by executing the
following commands as root:

.. code-block:: console

    # wget -O - https://nightly.incore.co/incore.key | apt-key add -
    # echo "deb http://nightly.incore.co/12.0/nightly/deb/ ./" >> /etc/apt/sources.list.d/incore.list
    # apt-get update && apt-get install incore
...
This will install inCore as a service, create the necessary PostgreSQL_ user
...
.. warning:: Debian 9 and Ubuntu do not provide a package for the python module
             num2words.
             Textual amounts will not be rendered by inCore and this could cause
...
inCore 12.0 'rpm' package supports Fedora 26.
As of 2017, CentOS does not have the minimum Python requirements (3.5) for
inCore 12.0.

Prepare
^^^^^^^
inCore needs a `PostgreSQL`_ server to run properly. Assuming that the 'sudo'
...
inCore. provides a repository that can be used with the Fedora distibutions.
It can be used to install inCore Community Edition by executing the following
commands:

.. code-block:: console

    $ sudo dnf config-manager --add-repo=https://nightly.incore.co/12.0/nightly/rpm/incore.repo
    $ sudo dnf install -y incore
    $ sudo systemctl enable incore
    $ sudo systemctl start incore
...
.. code-block:: console
    ...
    $ sudo dnf localinstall incore_12.0.latest.noarch.rpm
    $ sudo systemctl enable incore
    $ sudo systemctl start incore
...
The source "installation" really is about not installing inCore, and running
it directly from source instead.

This can be more convenient for module developers as the inCore source is
...
It also makes starting and stopping inCore more flexible and explicit than the
...
easily keeping (and running) multiple versions of inCore side-by-side.
...
* Python 3.5+.
  ...
  - on Windows, use `the official Python 3 installer
    ...
    .. note:: if Python is already installed, make sure it is 3.5 or above,
              previous versions are not compatible with inCore.

* PostgreSQL, to use a local database

  After installation you will need to create a postgres user: by default the
  only user is ``postgres``, and inCore forbids connecting as ``postgres``.
  ...
  - on Windows, use `PostgreSQL for windows`_ then
    ...
    - create a postgres user with a password using the pg admin gui: open
      ...
      usename in the :guilabel:`Role Name` field (e.g. ``incore``), then open
      the :guilabel:`Definition` tab and enter the password (e.g. ``incore``),
      then click :guilabel:`OK`.

      The user and password must be passed to inCore using either the
      :option:`-w <incore-bin -w>` and :option:`-r <incore-bin -r>` options or
...
* Python dependencies listed in the :file:`requirements.txt` file.
  ...
  - on Windows you need to install some of the dependencies manually, tweak the
    ...
    command from a cmd.exe prompt (replace ``\YourinCorePath`` by the actual
    path where you downloaded inCore):

    .. code-block:: doscon

        C:\> cd \YourinCorePath
        C:\YourinCorePath> C:\Python35\Scripts\pip.exe install -r requirements.txt
...
There are two ways to obtain the inCore source code: zip or git.

* inCore zip can be downloaded from  our nightly_ server or our Download_  page,
...
* git allows simpler update and easier switching between different versions
  of inCore. It also simplifies maintaining non-module patches and
  contributions.  The primary drawback of git is that it is significantly
  larger than a tarball as it contains the entire history of the inCore project.

Community Edition
'''''''''''''''''

The git repository is https://github.com/incore/incore.git for the Community
edition.
...
.. code-block:: console
    ...
    $ git clone https://github.com/incore/incore.git
...
.. code-block:: console
  ...
  $ git clone https://github.com/incore/enterprise.git

.. note:: The Enterprise git repository **does not contain the full inCore
...
Once all dependencies are set up, inCore can be launched by running ``incore-bin``.
...
Under Windows a typical way to execute incore would be:

.. code-block:: doscon

    C:\YourinCorePath> python3 incore-bin -w incore -r incore --addons-path=addons,../mymodules --db-filter=mydb$

Where ``incore``, ``incore`` are the postgresql login and password,
``../mymodules`` a directory with additional addons and ``mydb`` the default
db to serve on localhost:8080

Under Unix a typical way to execute incore would be:

.. code-block:: console

    $ ./incore-bin --addons-path=addons,../mymodules --db-filter=mydb$

Where ``../mymodules`` is a directory with additional addons and ``mydb`` the
default db to serve on localhost:8080
...
This section will explain how to run inCore in a such isolated Python environment.
...
This section assumes that you obtained the inCore sources from the zip file or the
...
Now, let's install the tools required to build inCore dependencies if needed:
...
Now we can create a virtual environment for inCore like this:

.. code-block:: console

  $ mkvirtualenv -p /usr/bin/python3 incore-venv

With this command, we ask for an isolated Python3 environment that will be named
"incore-env". If the command works as expected, your shell is now using this
...
Now let's install the inCore required python packages:

.. code-block:: console

  $ cd your_incore_sources_path
  $ pip install -r requirements.txt

After a little while, you should be ready to run incore from the command line as
explained above.
...
Whenever you want to work again with your 'incore-venv' environment:
...
The full documentation on how to use inCore with Docker can be found on the
official inCore `docker image <https://registry.hub.docker.com/_/incore/>`_ page.

.. _demo: https://demo.incore.co
.. _docker: https://www.docker.com
.. _Download: https://www.incore.co/page/download
...
.. _saas: https://www.incore.co/page/start
...
.. _the repository: https://github.com/incore/incore
.. _git: http://git-scm.com
.. _Editions: https://www.incore.co/pricing#pricing_table_features
.. _nightly: https://nightly.incore.co/12.0/nightly/
.. _extra: https://nightly.incore.co/extra/
