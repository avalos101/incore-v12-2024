:banner: banners/deploying_incore.jpg

==============
Deploying inCore
==============

This document describes basic steps to set up inCore in production or on an
...
inCore is a multi-tenant system: a single inCore system may run and serve a number
...
bound to a database: inCore needs to know which database should be used to load
...
accessible inCore needs a rule to know which one it should use.

That is one of the purposes of :option:`--db-filter <incore-bin --db-filter>`:
...
.. note::
  Setting a proper :option:`--db-filter <incore-bin --db-filter>` is an important part
...
UNIX socket is fine if you want inCore and PostgreSQL to execute on the same
machine, and is the default when no host is provided, but if you want inCore and
...
* Only accept loopback connections and `use an SSH tunnel`_ between the
  machine on which inCore runs and the one on which PostgreSQL runs, then
  configure inCore to connect to its end of the tunnel
* Accept connections to the machine on which inCore is installed, possibly
  over ssl (see `PostgreSQL connection settings`_ for details), then configure
  inCore to connect over the network
...
.. _setup/deploy/incore:

Configuring inCore
----------------

Out of the box, inCore connects to a local postgres over UNIX socket via port
...
create a new user (``incore``) and set it as the database user.
...
SSL Between inCore and PostgreSQL
-------------------------------

Since inCore 11.0, you can enforce ssl connection between inCore and PostgreSQL.
in inCore the db_sslmode control the ssl security of the connection
...
inCore includes built-in HTTP servers, using either multithreading or
multiprocessing.
...
* Multiprocessing is enabled by configuring :option:`a non-zero number of
  worker processes <incore-bin --workers>`, the number of workers should be based
...
listening on :option:`the longpolling port <incore-bin --longpolling-port>` but
the client will not connect to it.

Instead you must have a proxy redirecting requests whose URL starts with
``/longpolling/`` to the longpolling port. Other request should be proxied to
the :option:`normal HTTP port <incore-bin --http-port>`

To achieve such a thing, you'll need to deploy a reverse proxy in front of inCore,
like nginx or apache. When doing so, you'll need to forward some more http Headers
to inCore, and activate the proxy_mode in inCore configuration to have inCore read those
headers.
...
* RAM = 9 * ((0.8*150) + (0.2*1024)) ~= 3Go RAM for inCore
...
Whether it's accessed via website/web client or web service, inCore transmits
authentication information in cleartext. This means a secure deployment of
inCore must use HTTPS\ [#switching]_. SSL termination can be implemented via
just about any SSL termination proxy, but requires the following setup:

* Enable inCore's :option:`proxy mode <incore-bin --proxy-mode>`. This should only be enabled when inCore is behind a reverse proxy
...
in ``/etc/nginx/sites-enabled/incore.conf`` set:
...
.. code-block:: nginx
  ...
  server {
     listen 80;
     server_name incore.mycompany.com;
  ...
  server {
   ...
   # Add Headers for incore proxy mode
   ...
   # log
   access_log /var/log/nginx/incore.access.log;
   error_log /var/log/nginx/incore.error.log;

   # Redirect longpoll requests to incore longpolling port
   location /longpolling {
   proxy_pass http://incorechat;
   }

   # Redirect requests to incore backend server
   location / {
     proxy_redirect off;
     proxy_pass http://incore;
   }
...
inCore as a WSGI Application
==========================

It is also possible to mount inCore as a standard WSGI_ application. inCore
provides the base for a WSGI launcher script as ``incore-wsgi.example.py``. That
script should be customized (possibly after copying it from the setup directory) to correctly set the
configuration directly in :mod:`incore.tools.config` rather than through the
command-line or a configuration file.

However the WSGI server will only expose the main HTTP endpoint for the web
client, website and webservice API. Because inCore does not control the creation
of workers anymore it can not setup cron or livechat workers

Cron Workers
------------

To run cron jobs for an inCore deployment as a WSGI application requires

* A classical inCore (run via ``incore-bin``)
...
* Which should not be exposed to the network. To ensure cron runners are not
  ...
  entirely with :option:`incore-bin --no-http` or setting ``http_enable = False``
...
* Deploy a threaded version of inCore (instread of a process-based preforking
  one) and redirect only requests to URLs starting with ``/longpolling/`` to
  that inCore, this is the simplest and the longpolling URL can double up as
  the cron instance.
* Deploy an evented inCore via ``incore-gevent`` and proxy requests starting
  with ``/longpolling/`` to
  :option:`the longpolling port <incore-bin --longpolling-port>`.

Serving Static Files
====================

For development convenience, inCore directly serves all static files in its
...
inCore static files live in each module's ``static/`` folder, so static files
...
- Use appropriate database filters ( :option:`--db-filter <incore-bin --db-filter>`)
  to restrict the visibility of your databases according to the hostname.
  See :ref:`db_filter`.
  You may also use :option:`-d <incore-bin -d>` to provide your own (comma-separated)
...
- Once your ``db_name`` and ``db_filter`` are configured and only match a single database
  ...
  (this is also exposed as the :option:`--no-database-list <incore-bin --no-database-list>`
  command-line option)

- Make sure the PostgreSQL user (:option:`--db_user <incore-bin --db_user>`) is *not* a super-user,
  ...
  See also :ref:`setup/deploy/incore`.
...
- Keep installations updated by regularly installing the latest builds,
  ...
  https://www.incore.co/page/download or http://nightly.incore.co
...
- Run inCore behind a web server providing HTTPS termination with a valid SSL certificate,
  ...
  and then enable the :option:`proxy mode <incore-bin --proxy-mode>` option.
...
threat should not be neglected for inCore servers. inCore emits a log entry whenever a login attempt
...
Failed login::
      ...
      2018-07-05 14:56:31,506 24849 INFO db_name incore.addons.base.res.res_users: Login failed for db:db_name login:admin from 127.0.0.1

Successful login::

      2018-07-05 14:56:31,506 24849 INFO db_name incore.addons.base.res.res_users: Login successful for db:db_name login:admin from 127.0.0.1
...
10 failed login attempts are detected from the same IP within 1 minute::
    ...
    logpath = /var/log/incore.log  ;  set the actual incore log path here
...
:ref:`setup/deploy/incore` mentioned ``admin_passwd`` in passing.
...
inCore is supported by multiple browsers for each of its versions. No
distinction is made according to the browser version in order to be
up-to-date. inCore is supported on the current browser version. The list
...
.. [#different-machines]
    to have multiple inCore installations use the same PostgreSQL database,
...
.. _POSBox: https://www.incore.co/page/point-of-sale-hardware#part_2
