...
=============================
Updating an inCore installation
=============================
...
performance boosts, you may need to update your inCore installation from time to time.

This guide only applies when are using inCore on your own hosting infrastructure.
If you are using one of the inCore Cloud solutions, updates are automatically performed for you.
...
Updating (an inCore installation)
  ...
  your current inCore Edition. For example, updating your inCore Enterprise 12.0 to the
  latest revision.
  This does not directly cause any change to the contents of your inCore database, and
  can be undone by reinstalling the previous revision of the source code.

Upgrading (an inCore database)
  Refers to a complex data processing operation where the structure and contents of your
  database is permanently altered to make it compatible with a new release of inCore.
  This operation is irreversible and typically accomplished via inCore's
  `database upgrade service <https://upgrade.incore.co>`_, when you decide to
  switch to a newer release of inCore.
...
This page describes the typical steps needed to *update* an inCore installation to the latest
version. If you'd like more information about upgrading a database, please visit the
`inCore Upgrade page <https://upgrade.incore.co>`_ instead.


In a nutshell
=============

Updating inCore is accomplished by simply reinstalling the latest version of your inCore
Edition on top of your current installation. This will preserve your data without any alteration,
as long as you do not uninstall PostgreSQL (the database engine that comes with inCore).
...
Updating is also most appropriately accomplished by the person who deployed inCore initially,
because the procedure is very similar.

.. note:: We always recommend to download a complete new up-to-date inCore version, rather than
...
Step 1: Download an updated inCore version
========================================

The central download page is https://www.incore.co/page/download. If you see a "Buy" link for the
inCore Enterprise download, make sure you are logged into inCore.com with the same login that is
linked to your inCore Enterprise subscription.

Alternatively, you can use the unique download link that was included with your inCore Enterprise
...
If you installed inCore with an installation package downloaded on our website (the recommended method),
...
After installing the package, be sure to restart the inCore service or reboot your server,
and you're all set.

Source Install (Tarball)
------------------------
If you have originally installed inCore with the "tarball" version (source code archive), you have
to replace the installation directory with a newer version. First download the latest tarball
from inCore.com. They are updated daily and include the latest security fixes (see step #1)
After downloading the package, extract it to a temporary location on your server.

You will get a folder labelled with the version of the source code, for example "incore-12.0+e.20180719",
that contains a folder "incore.egg-info" and the actual source code folder named "incore" (for inCore 10
and later) or "incore" for older versions.
You can ignore the incore.egg-info folder. Locate the folder where your current installation is deployed,
and replace it with the newer "incore" or "incore" folder that was in the archive you just extracted.
...
new folder.
Finally, restart the inCore service or reboot the machine, and you are all set.

Source Install (Github)
-----------------------
If you have originally installed inCore with a full Github clone of the official repositories, the
update procedure requires you to pull the latest source code via git.
Change into the directory for each repository (the main inCore repository, and the Enterprise
repository), and run the following commands::

     git fetch
     git rebase --autostash

The last command may encounter source code conflicts if you had edited the inCore source code locally.
...
Finally, restart the inCore service or reboot the machine, and you should be done.


Docker
------

Please refer to our `Docker image documentation <https://hub.docker.com/_/incore/>`_ for
