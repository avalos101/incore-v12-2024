...
inCore is usually extended internally via modules, but many of its features and
all of its data are also available from the outside for external analysis or
integration with various tools. Part of the :ref:`reference/orm/model` API is
easily available over XML-RPC_ and accessible from a variety of languages.

.. inCore XML-RPC idiosyncracies:
   * uses multiple endpoint and a nested call syntax instead of a
     "hierarchical" server structure (e.g. ``incore.res.partner.read()``)
...
.. only:: html
    ...
    .. rst-class:: setupcode hidden
        ...
        .. code-block:: python3
            ...
            info = xmlrpc.client.ServerProxy('https://demo.incore.co/start').start()
        ...
        .. code-block:: ruby
            ...
            info = XMLRPC::Client.new2('https://demo.incore.co/start').call('start')
            url, db, username, password = \
                info['host'], info['database'], info['user'], info['password']
        ...
        .. code-block:: php
            ...
            $info = ripcord::client('https://demo.incore.co/start')->start();
        ...
        .. code-block:: java
            ...
            start_config.setServerURL(new URL("https://demo.incore.co/start"));
...
If you already have an inCore server installed, you can just use its
parameters

.. warning::

    For inCore Online instances (<domain>.incore.co), users are created without a
    *local* password (as a person you are logged in via the inCore Online
    authentication system, not by the instance itself). To use XML-RPC on inCore
    ...
    *https://mycompany.incore.co*), the *database name* is the name of the
...
To make exploration simpler, you can also ask https://demo.incore.co for a test
database:
...
inCore requires users of the API to be authenticated before they can query most 
data.
...
The second endpoint is ``xmlrpc/2/object``, is used to call methods of incore
...
Records can be listed and filtered via :meth:`~incore.models.Model.search`.

:meth:`~incore.models.Model.search` takes a mandatory
...
:meth:`~incore.models.Model.search_count` can be used to retrieve
only the number of records matching the query. It takes the same
:ref:`domain <reference/orm/domains>` filter as
:meth:`~incore.models.Model.search` and no other parameter.
...
Record data is accessible via the :meth:`~incore.models.Model.read` method,
which takes a list of ids (as returned by
:meth:`~incore.models.Model.search`) and optionally a list of fields to
...
:meth:`~incore.models.Model.fields_get` can be used to inspect
...
Because it is a very common task, inCore provides a
:meth:`~incore.models.Model.search_read` shortcut which as its name suggests is
equivalent to a :meth:`~incore.models.Model.search` followed by a
:meth:`~incore.models.Model.read`, but avoids having to perform two requests
and keep ids around.

Its arguments are similar to :meth:`~incore.models.Model.search`'s, but it
can also take a list of ``fields`` (like :meth:`~incore.models.Model.read`,
...
Records of a model are created using :meth:`~incore.models.Model.create`. The
method will create a single record and return its database identifier.

:meth:`~incore.models.Model.create` takes a mapping of fields to values, used
...
.. warning::
    ...
    :class:`~incore.fields.Integer`, string for :class:`~incore.fields.Char`
    or :class:`~incore.fields.Text`),

    * :class:`~incore.fields.Date`, :class:`~incore.fields.Datetime` and
      :class:`~incore.fields.Binary` fields use string values
    * :class:`~incore.fields.One2many` and :class:`~incore.fields.Many2many`
      use a special command protocol detailed in :meth:`the documentation to
      the write method <incore.models.Model.write>`.

Update records
--------------

Records can be updated using :meth:`~incore.models.Model.write`, it takes
a list of records to update and a mapping of updated fields to values similar
to :meth:`~incore.models.Model.create`.
...
:meth:`~incore.models.Model.unlink`.
...
While we previously used :meth:`~incore.models.Model.fields_get` to query a
model and have been using an arbitrary model from the start, inCore stores
...
Provides information about inCore models via its various fields
...
``field_id``
    list of the model's fields through a :class:`~incore.fields.One2many` to
    :ref:`reference/webservice/inspection/fields`
``view_ids``
    :class:`~incore.fields.One2many` to the :ref:`reference/views` defined
    for the model
``access_ids``
    :class:`~incore.fields.One2many` relation to the
...
Provides information about the fields of inCore models and allows adding
custom fields without using Python code

``model_id``
    :class:`~incore.fields.Many2one` to
...
``groups``
    ...
    :class:`~incore.fields.Many2many` to ``res.groups``
