...
class PostgreSQLHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        if not dbname:
            return
        with tools.ignore(Exception), tools.mute_logger('incore.sql_db'), sql_db.db_connect(dbname, allow_uri=True).cursor() as cr:
            cr.autocommit(True)
...
def init_logger():
    ...
    # Check that handler.stream has a fileno() method: when running inCore
...
DEFAULT_LOG_CONFIGURATION = [
    'incore.http.rpc.request:INFO',
    'incore.http.rpc.response:INFO',
    ':INFO',
]
PSEUDOCONFIG_MAPPER = {
    'debug_rpc_answer': ['incore:DEBUG', 'incore.sql_db:INFO', 'incore.http.rpc:DEBUG'],
    'debug_rpc': ['incore:DEBUG', 'incore.sql_db:INFO', 'incore.http.rpc.request:DEBUG'],
    'debug': ['incore:DEBUG', 'incore.sql_db:INFO'],
    'debug_sql': ['incore.sql_db:DEBUG'],
    'info': [],
    'warn': ['incore:WARNING', 'werkzeug:WARNING'],
    'error': ['incore:ERROR', 'werkzeug:ERROR'],
    'critical': ['incore:CRITICAL', 'werkzeug:CRITICAL'],
}
