# -*- coding: utf-8 -*-
"""
The module :mod:`incore.tests.common` provides unittest test cases and a few
...
from incore.models import BaseModel
from incore.osv.expression import normalize_domain
from incore.tools import pycompat
from incore.tools import single_email_re
from incore.tools.misc import find_in_path
...
from incore.service import security



_logger = logging.getLogger(__name__)

# The incore library is supposed already configured.
ADDONS_PATH = incore.tools.config['addons_path']
HOST = '127.0.0.1'
PORT = incore.tools.config['http_port']
# Useless constant, tests are aware of the content of demo data
ADMIN_USER_ID = incore.SUPERUSER_ID


def get_db_name():
    db = incore.tools.config['db_name']
...
def new_test_user(env, login='', groups='base.group_user', context=None, **kwargs):
     ...
     * email: it is either the login (if it is a valid email) or a generated
       ...
       to email being required for most incore operations;
    """
    if not login:
...
class MetaCase(type):
    ...
    def __init__(cls, name, bases, attrs):
        ...
        if cls.__module__.startswith('incore.addons.'):
...
class BaseCase(TreeCase, MetaCase('DummyCase', (object,), {})):
    """
    Subclass of TestCase for common inCore-specific code.

    This class is abstract and expects self.registry, self.cr and self.uid to be
    initialized by subclasses.
    """

    longMessage = True      # more verbose error message by default: https://www.incore.co/r/Vmh
    ...
    def browse_ref(self, xid):
        ...
        :param xid: fully-qualified :term:`external identifier`, in the form
                    :samp:`{module}.{identifier}`
        :raise: ValueError if not found
        :returns: :class:`~incore.models.BaseModel`
        """
        ...
        if self.warm:
            ...
            with self.subTest(), patch('random.random', lambda: 1):
                ...
                if count != expected:
                    ...
                    if "/incore/addons/" in filename:
                        filename = filename.rsplit("/incore/addons/", 1)[1]
                    if count > expected:
...
class TransactionCase(BaseCase):
    ...
    def setUp(self):
        ...
        self.registry = incore.registry(get_db_name())
        #: current transaction's cursor
        self.cr = self.cursor()
        #: :class:`~incore.api.Environment` for the current test case
        self.env = api.Environment(self.cr, incore.SUPERUSER_ID, {})
...
class SingleTransactionCase(BaseCase):
    ...
    def setUpClass(cls):
        ...
        cls.registry = incore.registry(get_db_name())
        cls.cr = cls.registry.cursor()
        cls.env = api.Environment(cls.cr, incore.SUPERUSER_ID, {})
...
class ChromeBrowser():
    ...
    def __init__(self, logger):
        ...
        self.request_id = 0
        self.user_data_dir = tempfile.mkdtemp(suffix='_chrome_incore')
    ...
    def _get_shotname(self, prefix, ext):
        ...
        base_file = os.path.splitext(incore.tools.config['logfile'])[0]
        name = '%s_%s_%s.%s' % (base_file, prefix, timestamp, ext)
        return name

    def take_screenshot(self, prefix='failed'):
        if not incore.tools.config['logfile']:
    ...
    def _save_screencast(self, prefix='failed'):
        ...
        if not incore.tools.config['logfile']:
            ...
            return None
        sdir = tempfile.mkdtemp(suffix='_chrome_incore_screencast')
        nb = 0
...
class HttpCase(TransactionCase):
    ...
    def setUp(self):
        ...
        self.session = incore.http.root.session_store.new()
        self.session_id = self.session.sid
        self.session.db = get_db_name()
        incore.http.root.session_store.save(self.session)
    ...
    def _wait_remaining_requests(self):
        ...
        for thread in threading.enumerate():
            if thread.name.startswith('incore.service.http.request.'):
                join_retry_count = 10
    ...
    def authenticate(self, user, password):
        ...
        # inCoreSession.authenticate accesses the current request, which we
        ...
        incore.http.root.session_store.save(session)
        if self.browser:
    ...
    def browser_js(self, url_path, code, ready='', login=None, timeout=60, **kw):
        ...
        try:
            ...
            if incore.tools.config['logfile']:
...
class Form(object):
    ...
    * properly handle defaults & onchanges around x2many fields
    ...
    :class:`~incore.fields.Many2one` fields assign a singleton recordset::
    ...
    For :class:`~incore.fields.Many2many` fields, the field itself is a
    :class:`~incore.tests.common.M2MProxy` and can be altered by adding or
    removing records::
    ...
    Finally :class:`~incore.fields.One2many` are reified as
    :class:`~incore.tests.common.O2MProxy`.

    Because the :class:`~incore.fields.One2many` only exists through its
    parent, it is manipulated more directly by creating "sub-forms"
    with the :meth:`~incore.tests.common.O2MProxy.new` and
    :meth:`~incore.tests.common.O2MProxy.edit` methods. These would
    ...
    :type recordp: incore.models.Model
    ...
    :type view: int | str | incore.model.Model
    ...
    def _init_from_defaults(self, model):
        ...
        def cleanup(k, v):
            if fields[k]['type'] == 'one2many':
                ...
                return incore.fields.Datetime.to_string(v)
...
class O2MProxy(X2MProxy):
    ...
    def new(self):
        ...
        :class:`~incore.fields.One2many` record, properly initialised.
    ...
    def edit(self, index):
        ...
        :class:`~incore.fields.One2many` record.
...
def record_to_values(fields, record):
    ...
    for f, descr in fields.items():
        ...
        if descr['type'] == 'many2one':
            ...
            v = incore.fields.Datetime.to_string(v)
        elif descr['type'] == 'date' and isinstance(v, date):
            v = incore.fields.Date.to_string(v)
        r[f] = v
    return r
...
def tagged(*tags):
    ...
    By default, all Test classes from incore.tests.common have a test_tags
