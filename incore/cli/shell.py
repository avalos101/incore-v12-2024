...
class Shell(Command):
    """Start incore in an interactive shell"""
    supported_shells = ['ipython', 'ptpython', 'bpython', 'python']

    def init(self, args):
        config.parse_config(args)
        incore.cli.server.report_configuration()
        incore.service.server.start(preload=[], stop=True)
        ...
        with incore.api.Environment.manage():
            if dbname:
                ...
                with registry.cursor() as cr:
                    ...
                    ctx = incore.api.Environment(cr, uid, {})['res.users'].context_get()
                    env = incore.api.Environment(cr, uid, ctx)
                    local_vars['env'] = env
