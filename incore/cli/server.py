...
"""
inCore - Server
inCore is an ERP+CRM program for small and medium businesses.

The whole source code is distributed under the terms of the
GNU Public Licence.

(c) 2003-TODAY, Fabien Pinckaers - inCore SA
"""
...
__author__ = incore.release.author
__version__ = incore.release.version

# Also use the `incore` logger for the main script.
_logger = logging.getLogger('incore')
...
def report_configuration():
    ...
    """
    config = incore.tools.config
    _logger.info("inCore version %s", __version__)
    if os.path.isfile(config.rcfile):
        _logger.info("Using configuration file at " + config.rcfile)
    _logger.info('addons paths: %s', incore.modules.module.ad_paths)
...
def setup_pid_file():
    ...
    """
    config = incore.tools.config
    if not incore.evented and config['pidfile']:
        pid = os.getpid()
...
def export_translation():
    ...
    with open(config["translate_out"], "wb") as buf:
        registry = incore.modules.registry.Registry.new(dbname)
        with incore.api.Environment.manage():
            with registry.cursor() as cr:
                incore.tools.trans_export(config["language"],
...
def import_translation():
    ...
    with incore.api.Environment.manage():
        with registry.cursor() as cr:
            incore.tools.trans_load(
                cr, config["translate_in"], config["language"], context=context,
            )

def main(args):
    check_root_user()
    incore.tools.config.parse_config(args)
    check_postgres_user()
    ...
    if config['db_name']:
        ...
        for db_name in preload:
            try:
                incore.service.db._create_empty_database(db_name)
                ...
                if err.pgcode == errorcodes.INSUFFICIENT_PRIVILEGE:
                    ...
                    raise err
            except incore.service.db.DatabaseExists:
                pass
    ...
    if config['workers']:
        incore.multi_process = True

    stop = config["stop_after_init"]

    setup_pid_file()
    rc = incore.service.server.start(preload=preload, stop=stop)
    sys.exit(rc)

class Server(Command):
    """Start the incore server (default command)"""
    def run(self, args):
        main(args)
